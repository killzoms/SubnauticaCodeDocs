    ...
    public class GraphicsDebugGUI : MonoBehaviour
    {
        ...
        private class ComponentToggle<C> : GraphicsOption where C : MonoBehaviour
        {
            ...
            public void OnGUI(char hotkey)
            {
                ...
                if (toggleObject)
                {
                    if (global::UWE.Utils.ToggleChanged(comp.gameObject.activeInHierarchy, hotkey + " :: " + label) || HotkeyDown(hotkey))
                    {
                ...
                else if (global::UWE.Utils.ToggleChanged(comp.enabled, hotkey + " :: " + label) || HotkeyDown(hotkey))
                {
        ...
        private void Start()
        {
            ...
            if (num < array.Length)
            {
                WaterscapeVolume waterscapeVolume = (fog = array[num]);
            }
            WaterscapeVolumeOnCamera[] array2 = Object.FindObjectsOfType<WaterscapeVolumeOnCamera>();
            num = 0;
            if (num < array2.Length)
            {
                WaterscapeVolumeOnCamera waterscapeVolumeOnCamera = (waterVolumeOnCamera = array2[num]);
            }
            WaterSurfaceOnCamera[] array3 = Object.FindObjectsOfType<WaterSurfaceOnCamera>();
            num = 0;
            if (num < array3.Length)
            {
                WaterSurfaceOnCamera waterSurfaceOnCamera = (this.waterSurfaceOnCamera = array3[num]);
            }
            FrameTimeOverlay[] array4 = Object.FindObjectsOfType<FrameTimeOverlay>();
            num = 0;
            if (num < array4.Length)
            {
                FrameTimeOverlay frameTimeOverlay = (measure = array4[num]);
            }
        }
        ...
        private void OnGUI()
        {
            ...
            DevConsole.disableConsole = PlayerPrefsUtils.PrefsToggle(DevConsole.disableConsole, "UWE.DisableConsole", "Disable Console");
            ...
            if (SNUtils.IsEngineDeveloper() || Application.isEditor)
            {
                GUILayout.BeginVertical("box");
