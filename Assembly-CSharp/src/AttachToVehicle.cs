...
namespace AssemblyCSharp
{
    ...
    public class AttachToVehicle : CreatureAction, IProtoEventListener, IPropulsionCannonAmmo, IOnTakeDamage
    {
        ...
        public override float Evaluate(Creature creature)
        {
            ...
            if (timeDetached + 4f < Time.time && currTarget != null && (currTarget.transform.position - base.transform.position).sqrMagnitude <= currTarget.distanceToStartAction * currTarget.distanceToStartAction)
            {
        ...
        public override void Perform(Creature creature, float deltaTime)
        {
            ...
            switch (state)
            {
                case State.SwimTo:
                {
                    ...
                    if (Time.time > timeNextSwim && currentAttachPoint != null)
                    {
