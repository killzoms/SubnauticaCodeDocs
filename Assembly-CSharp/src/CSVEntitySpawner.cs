...
namespace AssemblyCSharp
{
    public class CSVEntitySpawner : MonoBehaviour, LargeWorldEntitySpawner
    {
        ...
        public List<GameObject> GetPrefabsForSlot(IEntitySlot slot)
        {
            ...
            if (lootDistribution.GetBiomeLoot(slot.GetBiomeType(), out var data))
            {
                ...
                for (int count = data.prefabs.Count; i < count; i++)
                {
                    ...
                    if (srcDistribution.TryGetValue(prefabData.classId, out var _))
                    {
                        GameObject prefab;
                        ...
                        else if (slot.IsTypeAllowed(info.slotType) && PrefabDatabase.TryGetPrefab(prefabData.classId, out prefab))
                        {
                            list.Add(prefab);
                        }
                    }
