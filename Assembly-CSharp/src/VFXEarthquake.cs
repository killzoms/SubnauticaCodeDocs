...
namespace AssemblyCSharp
{
    public class VFXEarthquake : MonoBehaviour
    {
        ...
        private IEnumerator SpawnFX(Transform spawnPoint)
        {
            ...
            DeferredSpawner.Task task = DeferredSpawner.instance.InstantiateAsync(dustFXprefab[num], spawnPoint.position, spawnPoint.rotation);
            yield return task;
            GameObject result = task.result;
            DeferredSpawner.instance.ReturnTask(task);
            result.transform.parent = spawnPoint;
