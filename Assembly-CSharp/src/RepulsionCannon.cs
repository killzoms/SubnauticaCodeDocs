...
namespace AssemblyCSharp
{
    public class RepulsionCannon : PlayerTool, IEquippable
    {
        ...
        public override void OnToolUseAnim(GUIHand guiHand)
        {
            ...
            int num2 = global::UWE.Utils.SpherecastIntoSharedBuffer(position, 1f, forward, 35f, ~(1 << LayerMask.NameToLayer("Player")));
            float num3 = 0f;
            for (int i = 0; i < num2; i++)
            {
                ...
                iammo.Clear();
                if (flag && !(raycastHit.collider is MeshCollider) && (entityRoot.GetComponent<Pickupable>() != null || entityRoot.GetComponent<Living>() != null || (component.mass <= 1300f && global::UWE.Utils.GetAABBVolume(entityRoot) <= 400f)))
                {
