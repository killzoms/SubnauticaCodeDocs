...
namespace AssemblyCSharp
{
    ...
    public class LaunchRocket : HandTarget, IHandTarget
    {
            ...
            if (launchStarted)
            {
                ...
                for (int i = 0; i < subtitles.Length; i++)
                {
                    if (!subtitles[i].played && timePassed > subtitles[i].delay)
                    {
                        ...
                        Subtitles.main.Add(subtitles[i].name);
                        break;
                    }
                }
        ...
        public void OnHandHover(GUIHand hand)
        {
            ...
            if (flag)
            {
                flag = uGUI_SceneLoading.IsLoadingScreenFinished;
            }
            string interactInfo = (flag ? "Launch_Rocket" : "Rocket_NotReady");
            HandReticle.main.SetInteractInfo(interactInfo);
