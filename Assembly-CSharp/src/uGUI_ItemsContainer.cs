...
namespace AssemblyCSharp
{
    ...
    public class uGUI_ItemsContainer : MonoBehaviour, IDropHandler, IEventSystemHandler, uGUI_IIconManager, uGUI_INavigableIconGrid
    {
        ...
        private static readonly Dictionary<ItemsContainerType, Color> containerColors = new Dictionary<ItemsContainerType, Color>
        {
            {
                ItemsContainerType.Default,
                new Color(1f, 1f, 1f, 1f)
            },
            {
                ItemsContainerType.LandPlants,
                new Color(0.15f, 0.4f, 0.15f, 0.75f)
            },
            {
                ItemsContainerType.WaterPlants,
                new Color(0f, 0.18f, 0.36f, 0.8f)
            },
            {
                ItemsContainerType.Trashcan,
        ...
        private void OnAddItem(InventoryItem item)
        {
            ...
            uGUI_ItemIcon2.SetBackgroundRadius(33f);
        ...
        public void GetTooltip(uGUI_ItemIcon icon, out string tooltipText, List<TooltipIcon> tooltipIcons)
        {
            ...
            {
                tooltipText = inventory.GetTooltip(value);
            }
            else
            {
                tooltipText = null;
        ...
        public bool OnBeginDrag(uGUI_ItemIcon icon)
        {
            ...
            Vector2int itemSize = CraftData.GetItemSize(value.item.GetTechType());
            GetIconSize(itemSize.x, itemSize.y, out var width, out var height);
        ...
        public void SelectItem(object item)
        {
            ...
            {
                uGUI_Tooltip.Set(uGUI_ItemIcon2);
                UISelection.selected = uGUI_ItemIcon2;
                EquipmentType equipmentType = CraftData.GetEquipmentType(value.item.GetTechType());
