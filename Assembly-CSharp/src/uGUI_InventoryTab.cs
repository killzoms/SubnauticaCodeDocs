...
namespace AssemblyCSharp
{
    public class uGUI_InventoryTab : uGUI_PDATab
    {
        ...
        public string GetTooltip(InventoryItem item)
        {
            return TooltipFactory.InventoryItem(item);
        }
        ...
        public void OnPointerEnter(InventoryItem item)
        {
            if (!ItemDragManager.isDragging && Player.main.GetPDA().isInUse)
            {
                ...
                EquipmentType equipmentType = CraftData.GetEquipmentType(item.item.GetTechType());
            ...
            if (!ItemDragManager.isDragging)
            {
                switch (button)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        Inventory.main.AltUseItem(item);
                        break;
                }
                equipment.ExtinguishSlots();
            }
