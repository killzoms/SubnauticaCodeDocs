...
namespace AssemblyCSharp
{
    public class uGUI_IconNotifier : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (queue.Count > 0 && Time.time - time >= interval)
            {
                ...
                request.random = global::UnityEngine.Random.value;
                active.Add(request);
                time = Time.time;
            }
        ...
        private bool Animate(Request request)
        {
            ...
            request.t = Mathf.Clamp01(request.t + Time.deltaTime / iconAnimator.duration);
            return false;
        }
    }
}
