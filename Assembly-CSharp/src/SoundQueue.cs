    ...
    public class SoundQueue
    {
        ...
        public void Update()
        {
            ...
            if (eventInstance.isValid())
            {
                ...
                FMODUWE.CheckResult(eventInstance.set3DAttributes(attributes));
                FMODUWE.CheckResult(eventInstance.getTimelinePosition(out _position));
        ...
        public void Stop()
        {
            ...
            {
                FMODUWE.CheckResult(eventInstance.stop(FMOD.Studio.STOP_MODE.IMMEDIATE));
            }
            if (eventInstance.hasHandle())
            {
                FMODUWE.CheckResult(eventInstance.release());
        ...
        private void Play(string sound, string subtitles)
        {
            ...
            if (eventInstance.isValid())
            {
                ...
                _current = sound;
                FMODUWE.CheckResult(this.eventInstance.getDescription(out var description));
                FMODUWE.CheckResult(description.getLength(out _length));
                _lengthSeconds = (float)_length * 0.001f;
                FMODUWE.CheckResult(this.eventInstance.setVolume(1f));
                FMODUWE.CheckResult(this.eventInstance.start());
            }
        ...
        private static PLAYBACK_STATE GetPlaybackState(EventInstance evt)
        {
            ...
            if (evt.isValid())
            {
                FMODUWE.CheckResult(evt.getPlaybackState(out state));
            }
            return state;
        }
