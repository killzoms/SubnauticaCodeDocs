...
namespace AssemblyCSharp
{
    public class TrailManager : MonoBehaviour, IManagedLateUpdateBehaviour, IManagedBehaviour
    {
        ...
        {
            ProfilingUtils.BeginSample("TrailManager.Initialize");
            try
            {
        ...
        {
            ProfilingUtils.BeginSample("UpdateTrails");
            float scale = GetScale();
            ...
            for (int i = 0; i < trails.Length; i++)
            {
                ...
                Vector3 value = position - prevPositions[i];
                Vector3 vector2 = global::UWE.Utils.SlerpVector(prevPositions[i], position, Vector3.Normalize(value) * deltaTime * segmentSnapSpeed * scale);
                if (maxSegmentOffset > 0f && (position - vector2).sqrMagnitude > maxSegmentOffset * maxSegmentOffset)
                {
                    vector2 = position - value.normalized * maxSegmentOffset;
                }
                Vector3 normalized = trails[i].InverseTransformVector(vector - vector2).normalized;
                ...
                Vector3 vector3 = -trails[i].TransformDirection(trailSpaceForward[i]) * distances[i] * scale + vector;
                trails[i].position = vector3;
                prevPositions[i] = vector3;
