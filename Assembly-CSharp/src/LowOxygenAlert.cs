        ...
        private void Update()
        {
            ...
            if (!GameModeUtils.IsOptionActive(GameModeOption.NoHints) && (Utils.NearlyEqual(oxygenCapacity, lastOxygenCapacity) || oxygenCapacity < lastOxygenCapacity))
            {
                for (int num = alertList.Count - 1; num >= 0; num--)
                {
                    ...
                    if (oxygenCapacity >= alert.minO2Capacity && secondsMonitor.JustDroppedBelow(alert.oxygenTriggerSeconds) && Ocean.main.GetDepthOf(Utils.GetLocalPlayer()) > alert.minDepth && (player.IsSwimming() || (player.GetMode() == Player.Mode.LockedPiloting && !player.GetVehicle().IsPowered()) || (player.IsInSub() && !player.CanBreathe())))
                    {
