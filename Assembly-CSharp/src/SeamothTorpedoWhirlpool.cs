    ...
    public class SeamothTorpedoWhirlpool : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (sequence.active)
            {
                ...
                int num = global::UWE.Utils.OverlapSphereIntoSharedBuffer(tr.position, radiusPull, layerMask);
                for (int i = 0; i < num; i++)
                {
            ...
            for (int num = targets.Count - 1; num >= 0; num--)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            float num4 = Mathf.Lerp(pullMin, pullMax, num2 * num3) * force * Time.fixedDeltaTime;
                            float num5 = Mathf.Lerp(spinMin, spinMax, num2 * num3) * momentum * Time.fixedDeltaTime;
