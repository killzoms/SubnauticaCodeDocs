...
namespace AssemblyCSharp
{
    public class PrecursorActivatedPillar : MonoBehaviour, IStoryGoalListener, ICompileTimeCheckable
    {
        ...
        private bool SetGlowColor(Color color, bool onUpdate)
        {
            ...
            {
                Renderer renderer = illumRenderers[i];
                for (int j = 0; j < renderer.materials.Length; j++)
                {
                    Material material = renderer.materials[j];
                    if (material == null || !material.HasProperty(ShaderPropertyID._GlowColor))
                    {
                        continue;
                    }
                    Color value;
                    if (onUpdate)
                    {
                        value = Color.Lerp(material.GetColor(ShaderPropertyID._GlowColor), color, Time.deltaTime / dampening);
                    ...
                    material.SetColor(ShaderPropertyID._GlowColor, value);
        ...
        private void OnDestroy()
        {
            for (int i = 0; i < illumRenderers.Length; i++)
            {
                ...
                for (int j = 0; j < renderer.materials.Length; j++)
                {
                    global::UnityEngine.Object.Destroy(renderer.materials[j]);
                }
            }
            StoryGoalManager main = StoryGoalManager.main;
            if ((bool)main)
