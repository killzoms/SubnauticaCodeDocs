    ...
    public class SeaDragonMeleeAttack : MeleeAttack
    {
        ...
        public void OnTouchFront(Collider collider)
        {
            ...
            if (target.GetComponent<SubControl>() != null)
            {
                ...
                if (component != null)
                {
                    component.TakeDamage(shoveAttackDamage);
                }
            ...
            {
                if (BehaviourData.GetBehaviourType(target) != BehaviourType.Shark)
                {
                    return;
                }
                    ...
                    if (component3 != null)
                    {
                        if (component3.CanBeAttacked() && !component3.cinematicModeActive)
                        {
                            ...
                            if (component3.GetComponent<LiveMixin>().health - num <= 0f)
                            {
                                ...
                                component3.gameObject.GetComponent<LiveMixin>().TakeDamage(biteDamage);
                            }
                        }
                    }
                    else
        ...
        {
            if (liveMixin.IsAlive() && !isAttacking && !(Time.time < timeLastSwatAttack + swatAttackInterval) && !playerDeathCinematic.IsCinematicModeActive() && !seaDragon.IsHoldingExosuit() && target != null && BehaviourData.GetBehaviourType(target) == BehaviourType.Shark && CanBite(target))
            {
        ...
        public void OnSwatAttackHit()
        {
            ...
            if (component != null)
            {
                component.TakeDamage(swatAttackDamage);
            }
