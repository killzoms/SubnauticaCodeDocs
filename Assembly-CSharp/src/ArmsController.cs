...
namespace AssemblyCSharp
{
    ...
    public class ArmsController : MonoBehaviour
    {
        ...
        public IEnumerator InspectObjectAsync(InspectOnFirstPickup obj)
        {
            ...
            if ((bool)playerTool)
            {
                seconds = playerTool.holsterTime;
            }
            inspectObject = obj;
            if (Inventory.main.ReturnHeld())
            {
                ...
                inspectObject.gameObject.SetActive(value: true);
        ...
        {
            GetComponent<ConditionRules>().AddCondition(() => player.GetPDA().isInUse).WhenChanges(delegate(bool newValue)
            {
                SafeAnimator.SetBool(animator, "using_pda", newValue);
            });
        ...
        private void SetPlayerSpeedParameters()
        {
            ...
            smoothedVelocity = global::UWE.Utils.SlerpVector(smoothedVelocity, relativeVelocity, Vector3.Normalize(relativeVelocity - smoothedVelocity) * num * Time.deltaTime);
            ...
            float num2 = player.camRoot.GetAimingTransform().eulerAngles.x;
            if (num2 > 180f)
            {
                num2 -= 360f;
            }
            num2 = 0f - num2;
            SafeAnimator.SetFloat(animator, "view_pitch", num2);
        }
        ...
        private void UpdateHandIKWeights()
        {
            ...
            {
                positionWeight = global::UWE.Utils.Slerp(positionWeight, num, Time.deltaTime / ikToggleTime);
                positionWeight2 = global::UWE.Utils.Slerp(positionWeight2, num2, Time.deltaTime / ikToggleTime);
            }
        ...
        private void Update()
        {
            ProfilingUtils.BeginSample("ArmsController.Update()");
            ...
            {
                SafeAnimator.SetBool(animator, "using_tool", player.GetRightHandHeld());
                if (player.GetRightHandHeld())
                {
