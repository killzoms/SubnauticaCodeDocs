...
namespace AssemblyCSharp
{
    ...
    public class CollectShiny : CreatureAction, IProtoTreeEventListener, IManagedLateUpdateBehaviour, IManagedBehaviour
    {
        ...
        {
            if (timeNextFindShiny < Time.time)
            {
                UpdateShinyTarget();
                timeNextFindShiny = Time.time + updateTargetInterval * (1f + 0.2f * Random.value);
            }
        ...
        private void TryPickupShinyTarget()
        {
            if (shinyTarget != null && shinyTarget.activeInHierarchy)
            {
                ...
                global::UWE.Utils.SetCollidersEnabled(shinyTarget, enabled: false);
                shinyTarget.transform.parent = shinyTargetAttach;
                shinyTarget.transform.localPosition = Vector3.zero;
                targetPickedUp = true;
                global::UWE.Utils.SetIsKinematic(shinyTarget.GetComponent<Rigidbody>(), state: true);
                global::UWE.Utils.SetEnabled(shinyTarget.GetComponent<LargeWorldEntity>(), state: false);
        ...
        private void DropShinyTarget(GameObject target)
        {
            ...
            global::UWE.Utils.SetCollidersEnabled(target, enabled: true);
            global::UWE.Utils.SetIsKinematic(target.GetComponent<Rigidbody>(), state: false);
