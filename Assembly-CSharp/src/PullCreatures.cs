...
namespace AssemblyCSharp
{
    public class PullCreatures : CreatureAction
    {
        ...
        private bool LooksAtMe(GameObject target)
        {
            ...
            int num = global::UWE.Utils.RaycastIntoSharedBuffer(transform.position, vector, Vector3.Distance(base.transform.position, transform.position), -1, QueryTriggerInteraction.Ignore);
            for (int i = 0; i < num; i++)
            {
        ...
        public override float Evaluate(Creature creature)
        {
            if (!isActive)
            {
                if (Time.time > lastActionTime + pauseInterval && IsValidTarget(lastTarget.target))
                {
        ...
        public override void StartPerform(Creature behavior)
        {
            currentTarget = lastTarget.target;
            isActive = true;
            timeNextSwim = Time.time;
        ...
        public override void StopPerform(Creature behavior)
        {
            currentTarget = null;
            isActive = false;
        ...
        private void Update()
        {
            ProfilingUtils.BeginSample("CreatureAction::PullCreatures");
