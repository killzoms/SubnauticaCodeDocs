...
namespace AssemblyCSharp
{
    ...
    public class PlayerTimeCapsule : MonoBehaviour, IProtoTreeEventListener, IScreenshotClient, IProtoEventListener
    {
        ...
        private IEnumerator SubmitRoutine()
        {
            ...
            PlatformServices services = platformUtils.GetServices();
            if (services == null)
            {
            ...
            string timeCapsulePlatformGroup = GetTimeCapsulePlatformGroup(services.GetName());
            string sha256Hash = getSha256Hash(services.GetUserId());
        ...
        private IEnumerator SpawnRoutine()
        {
            ...
            string timeCapsulePlatformGroup = GetTimeCapsulePlatformGroup(services.GetName());
            string text = services.GetUserId();
            string currentLanguage = Language.main.GetCurrentLanguage();
            ...
            if (Application.isEditor && string.IsNullOrEmpty(text))
            {
                text = "editor";
            }
            if (string.IsNullOrEmpty(text))
            ...
            string s = string.Format("{0}={1}&{2}={3}&{4}={5}&{6}={7}", "platform", Uri.EscapeDataString(timeCapsulePlatformGroup), "platform_user_id", Uri.EscapeDataString(text), "language", Uri.EscapeDataString(currentLanguage), "play_time", num.ToString());
            ...
            while (spawnQueue.Count > 0)
            {
                ...
                while (attempt <= spawnRetryDelays.Length)
                {
                    ...
                    using (UnityWebRequest request = MakePostRequest("https://subnautica.unknownworlds.com/api/v1/time-capsule-spawn", payload, "application/x-www-form-urlencoded", 15))
                    {
                        ...
                        if (!request.isNetworkError)
                        {
                            ...
                            if (!string.IsNullOrEmpty(text3))
                            {
                                if (request.responseCode == 201)
                                {
                                    ...
                                    {
                                        text2 = $"Exception during parsing sever response Json: '{ex.ToString()}' responseText: '{text3}'";
                                    }
                                }
                            }
                            else
                        ...
                        if (!string.IsNullOrEmpty(text2))
                        {
                            Debug.LogErrorFormat("Time Capsule spawn error: '{0}', responseCode: '{1}'", text2, request.responseCode.ToString());
                        }
                    }
                    attempt++;
                    if (success)
        ...
        private IEnumerator OpenRoutine()
        {
            ...
            string platform = GetTimeCapsulePlatformGroup(services.GetName());
            string platformUserId = services.GetUserId();
