...
namespace AssemblyCSharp
{
    ...
    public class TileInstance : MonoBehaviour
    {
        ...
        public void CopyIntoRoot(VoxelandData dest, Int3 root)
        {
            ...
            VoxelandData.OctNode.BlendArgs args = new VoxelandData.OctNode.BlendArgs((blendMode != 0) ? ((blendMode != VoxelBlendMode.Additive) ? ((blendMode == VoxelBlendMode.Multiply) ? VoxelandData.OctNode.BlendOp.Intersection : ((blendMode == VoxelBlendMode.MultiplyKeepSource) ? VoxelandData.OctNode.BlendOp.Intersection : VoxelandData.OctNode.BlendOp.Union)) : VoxelandData.OctNode.BlendOp.Union) : VoxelandData.OctNode.BlendOp.Union, blendMode == VoxelBlendMode.Multiply, (blendMode == VoxelBlendMode.Multiply) ? GetTypeForDest(multiplyPassiveType) : Convert.ToByte(0));
            Int3.Bounds bounds = blockBounds;
        ...
        public void ClearRoot(VoxelandData dest, Int3 root)
        {
            ...
            {
                Vector3 p = item.ToVector3() + global::UWE.Utils.half3;
                VoxelandData.OctNode node = dest.GetNode(item.x, item.y, item.z);
        ...
        private GameObject CreateObjectPreview(GameObject prefabObj, Transform trans)
        {
            using (new StreamTiming.Block("CreateObjectPreview"))
            {
                ...
                GameObject gameObject = global::UnityEngine.Object.Instantiate(prefabObj);
                ...
                origPoint = global::UWE.Utils.CornerBoxRotatePoint(Vector3.zero, srcData.GetSize().ToVector3(), turns, origPoint);
        ...
        private void InEditorPreviewSpawn(GameObject prefab, bool bounded, Bounds bounds)
        {
            ...
            if (component2 != null)
            {
                ...
                {
                    if (PrefabDatabase.TryGetPrefab(component2.ClassId, out var prefabObj))
                    {
                        global::UnityEngine.Object.DestroyImmediate(CreateObjectPreview(prefabObj, prefab.transform).GetComponent<PrefabIdentifier>());
                        return;
                    }
            ...
            if (component3 != null)
            {
                if (!bounded || bounds.Contains(TileToGlobalPoint(prefab.transform.position)))
                {
                    global::UnityEngine.Object.DestroyImmediate(CreateObjectPreview(prefab, prefab.transform).GetComponent<StoreInformationIdentifier>());
                }
                return;
            }
        ...
        public void ClearEditorEntityPreview()
        {
            ...
            {
                if ((item.gameObject.hideFlags & HideFlags.NotEditable) > HideFlags.None)
                {
        ...
        public void Prepare(bool showPreviewEnts = true, bool bounded = false, Bounds entPreviewBounds = default(Bounds))
        {
            ...
            foreach (Voxeland voxeland in componentsInChildren)
            {
                ...
                ProfilingUtils.BeginSample("UpdateData");
            ...
            if (!Application.isPlaying && showPreviewEnts && prepared)
            {
                CreateEditorEntityPreview(bounded, entPreviewBounds);
            }
        }
        ...
        public void PreparePlayModeSpawns()
        {
            if (!(prefab == null))
            {
                ProfilingUtils.BeginSample("PreparePlayModeSpawns");
        ...
        public void SpawnPlayModeEntities(Bounds destBounds)
        {
            Utils.AssertEditMode("SpawnPlayModeEntities should only be called when caching in editor");
            if (prefab == null)
            {
                return;
            }
            if (!preparedPlayModeSpawns)
            {
                using (new StreamTiming.Block("SpawnPlayModeEntities::PreparePlayModeSpawns"))
                {
                    PreparePlayModeSpawns();
                }
            }
            Bounds bounds = wsBounds;
            bounds.Expand(new Vector3(1f, 1f, 1f));
            if (!bounds.Intersects(destBounds))
            {
                return;
            }
            ProfilingUtils.BeginSample("SpawnPlayModeEntities");
            foreach (KeyValuePair<UniqueIdentifier, PureTransform> item in ent2transform)
            {
                PureTransform value = item.Value;
                UniqueIdentifier key = item.Key;
                if (!destBounds.Contains(value.pos))
                {
                    continue;
                }
                ProfilingUtils.BeginSample("SpawnPlayModeEntities::Instantiate");
                PrefabIdentifier prefabIdentifier = key as PrefabIdentifier;
                GameObject gameObject2;
                if (prefabIdentifier != null)
                {
                    using (new StreamTiming.Block("SpawnPlayModeEntities::prefab"))
                    {
                        string classId = prefabIdentifier.ClassId;
                        if (!PrefabDatabase.TryGetPrefab(classId, out var gameObject))
                        {
                            Debug.LogError("Voxeland tile '" + base.gameObject.name + "' refers to a prefab with GUID '" + classId + "', but it is not in Assets/Resources/WorldEntities");
                            continue;
                        }
                        gameObject2 = global::UWE.Utils.InstantiateDeactivated(gameObject);
                        goto IL_018e;
                    }
                }
                using (new StreamTiming.Block("SpawnPlayModeEntities::oneoff"))
                {
                    GameObject obj = key.gameObject;
                    bool activeSelf = obj.activeSelf;
                    obj.SetActive(value: false);
                    gameObject2 = global::UnityEngine.Object.Instantiate(obj);
                    obj.SetActive(activeSelf);
                }
                goto IL_018e;
                IL_018e:
                Timer.Begin("SpawnTiming", 100f);
                using (new StreamTiming.Block("SpawnPlayModeEntities::instsetup"))
                {
                    gameObject2.transform.parent = null;
                    gameObject2.transform.localPosition = value.pos;
                    gameObject2.transform.localRotation = value.rot;
                    gameObject2.transform.localScale = value.scale;
                    gameObject2.AddComponent<TileInstanceEntity>();
                    using (new StreamTiming.Block("SpawnPlayModeEntities::enable"))
                    {
                    ...
                    using (new StreamTiming.Block("SpawnPlayModeEntities::register"))
                    {
                        world.streamer.cellManager.RegisterEntity(gameObject2);
                    }
                }
                float num = Timer.End();
                if (outputSpawnTimes)
                {
                    using StreamWriter streamWriter = File.AppendText("spawnTimings.csv");
                    streamWriter.WriteLine(gameObject2.gameObject.name + "," + num);
                }
                if (TileInstance.onSpawn != null)
                {
                    TileInstance.onSpawn(this, null);
        ...
        public static IEnumerable<Tile> EnumWorldTiles()
        {
            GameObject[] array = Resources.LoadAll<GameObject>("WorldTiles");
