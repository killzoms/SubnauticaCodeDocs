    ...
    public class ClipMapManager : MonoBehaviour
    {
        ...
        private class MeshingThread
        {
            ...
            public void Main()
            {
                try
                {
                    global::Platform.Utils.ThreadUtils.SetThreadName($"Meshing thread {index}");
                    global::Platform.Utils.ThreadUtils.SetThreadPriority(System.Threading.ThreadPriority.Lowest);
                    global::Platform.Utils.ThreadUtils.SetThreadAffinityMask(consoleAffinityMask);
                    MainLoop();
                    global::UnityEngine.Profiling.Profiler.EndThreadProfiling();
                }
                catch (Exception ex)
                {
                    global::UnityEngine.Debug.LogException(ex, mgr);
                    global::UnityEngine.Debug.LogErrorFormat("MeshingThread {0} is Dead due to exception: {1}!", index, ex);
                }
                finally
                {
                    state = State.Dead;
            ...
            private void MainLoop()
            {
                while (true)
                {
                    if (mgr.isDestroyed)
                    {
                        global::UnityEngine.Debug.LogWarning("ClipMapManager is destroyed, killing meshing thread!");
                        return;
                    }
                    ...
                    if (cell != null)
                    {
                        if (!cell.LockCell())
                        {
                            global::UnityEngine.Debug.LogErrorFormat("Meshing Thread {0} - Failed to lock cell {1}.", index, cell);
                            continue;
                        }
                        cell.OnDequeue();
                        state = State.Busy;
                ...
                global::UnityEngine.Debug.LogWarning("ClipMapManager.debugKillThreads is true, killing meshing thread!");
            }
        }
        ...
        private class Cell : IPriorityQueueItem
        {
            ...
            private float priorityUpdateRate = 71f / (339f * (float)System.Math.PI);
            ...
            public void HideMeshes()
            {
                if (!hidden)
                {
                    ProfilingUtils.BeginSample("HideMeshes");
                    if (debugForEdits)
                    {
                        global::UnityEngine.Debug.Log("Hiding chunk " + chunk.GetIndex());
                    }
                    hidden = true;
                    if (chunk != null)
                    {
            ...
            public void HideEntities()
            {
                if (!entsHidden)
                {
                    ProfilingUtils.BeginSample("HideEntities");
                    entsHidden = true;
            ...
            public void ShowMeshes()
            {
                if (hidden)
                {
                    ProfilingUtils.BeginSample("ShowMeshes");
                    if (debugForEdits)
                    {
                        global::UnityEngine.Debug.Log("Showing chunk " + chunk.GetIndex());
                    }
                    hidden = false;
            ...
            public void ShowEntities()
            {
                ...
                {
                    ProfilingUtils.BeginSample("ShowEntities");
                    entsHidden = false;
                    if (chunk != null && mgr.eventHandler != null && level.settings.entities)
                    {
            ...
            private void Unload(Int3 newChunkNum)
            {
                ...
                ProfilingUtils.BeginSample("Unload");
                HideMeshes();
                HideEntities();
                if (chunk.collision != null)
                {
            ...
            {
                ProfilingUtils.BeginSample("OnDirty");
                if (!chunk)
                {
                    CreateChunk();
                    chunk.cx = -1;
                ...
                if (Time.unscaledTime > nextCheckDataReadyTime && !SaveLoadManager.main.isSaving)
                {
                    ...
                    ProfilingUtils.BeginSample("IsChunkDataReady");
                    bool num = mgr.land.IsChunkDataReady(newChunkNum, level.settings.downsamples, size, level.settings.meshOverlap + 3);
                    ProfilingUtils.EndSample();
                    if (num)
                    {
                    ...
                    else
                    {
                        ...
                        nextCheckDataReadyTime = Time.unscaledTime + global::UnityEngine.Random.Range(0.5f, 1f);
                    }
                }
                ProfilingUtils.EndSample();
            }
            ...
            public void DoThreadablePart(MeshBufferPools bufferPools, VoxelandChunkWorkspace ws, VoxelandVisualMeshSimplifier _visSimp, VoxelandCollisionMeshSimplifier _colSimp, VoxelandGrassBuilder _grasser)
            {
                ...
                if (hasAnyGeometry)
                {
                    if (level.settings.colliders)
                    {
                        ...
                        ProfilingUtils.BeginSample("Collision Simplify");
                        colSimp.Build(ws);
                        ProfilingUtils.EndSample();
                    }
                    visSimp = _visSimp;
                    ...
                    ProfilingUtils.BeginSample("Visual Simplify");
                    ...
                    if (level.settings.grass && !mgr.debugOverrideDisableGrass)
                    {
                        ...
                        ProfilingUtils.BeginSample("Compute grass");
                ...
                if (hasAnyGeometry)
                {
                    ...
                    switch (finalizeStep)
                    {
                        case FinalizeStep.None:
                        {
                            ...
                            visSimp.BuildLayerObjects(chunk, level.settings.castShadows, addSortingValue, mgr.meshPool, mgr.streamer.chunkLayerPrefab);
                            visSimp.inUse = false;
                            visSimp = null;
                        ...
                        case FinalizeStep.MeshUploaded:
                            if (level.settings.colliders)
                            {
                                colSimp.AttachTo(chunk, mgr.meshPool);
                                colSimp.inUse = false;
                                colSimp = null;
                            }
                        ...
                        case FinalizeStep.CollisionUploaded:
                            if (level.settings.grass && !mgr.debugOverrideDisableGrass)
                            {
                                ProfilingUtils.BeginSample("Finalize Grass");
                                if (grasser != null)
                                {
                                    grasser.CreateUnityMeshes(chunk, mgr.meshPool);
                                    grasser.inUse = false;
                                    grasser = null;
                                }
                                ProfilingUtils.EndSample();
        ...
        private class Level
        {
            ...
            {
                ProfilingUtils.BeginSample("DebugDrawActiveChunks");
            ...
            {
                ProfilingUtils.BeginSample("NotifyBlocksUnloading");
            ...
            public void NotifyBlocksChanged(Int3.Bounds blocks)
            {
                ...
                ProfilingUtils.BeginSample("NotifyBlocksChanged");
                Int3.Bounds other = Int3.Bounds.OuterCoarserBounds(blocks, new Int3(blocksPerChunk));
                Int3.RangeEnumerator enumerator = activeChunks.IntersectionWith(other).GetEnumerator();
                while (enumerator.MoveNext())
                {
            ...
            {
                ProfilingUtils.BeginSample("UpdatePhase 2");
                foreach (Cell cell in cells)
                {
                    if (!cell.IsReady())
                    {
        ...
        public class ProcessFinalizeQueueCoroutine : StateMachineBase<ClipMapManager>
        {
        ...
        public class UpdateVisibilityForMeshingCoroutine : StateMachineBase<ClipMapManager>
        {
            ...
            public override bool MoveNext()
            {
                ...
                switch (visMeshingState)
                {
                    ...
                    case UpdateMeshVisState.SetLevel:
                        ...
                        {
                            ProfilingUtils.BeginSample("SetLevel");
                            if (host.AreMeshQueuesFull())
                            {
                                global::UnityEngine.Debug.LogWarningFormat("UpdateVisibilityForMeshingCoroutine- Mesh Queue is full!");
                                ProfilingUtils.EndSample();
                                return false;
                            }
                    ...
                    case UpdateMeshVisState.ProcessChunk:
                        ProfilingUtils.BeginSample("ProcessChunk");
                        numberResultsProcessed = 0;
                        while (rangeEnumerator.MoveNext())
                        {
        ...
        public class UpdateVisibilityForCellsCoroutine : StateMachineBase<ClipMapManager>
        {
        ...
        public void ReloadSettings()
        {
            ...
            global::UnityEngine.Debug.LogFormat("Read clip map settings from {0}:\n{1}", clipMapSettingsFileForQualityLevel, settings.ToString());
            int levelNum = 0;
        ...
        public void Initialize()
        {
            ...
            {
                num = settings.maxThreads;
                meshPool.poolingEnabled = false;
            }
            else
            {
                num = System.Math.Min(settings.maxThreads, Environment.ProcessorCount - Environment.ProcessorCount / 4);
            }
            ...
            {
                bufferPools[j] = new MeshBufferPools(settings.vertexBufferV2Max, settings.vertexBufferV3Max, settings.vertexBufferV4Max, settings.vertexBufferC32Max, settings.vertexBufferIndexMax);
            ...
            global::UnityEngine.Debug.Log("ClipMapManager initialize done");
        }
        ...
        public void LayoutDebugGUI()
        {
            if (GUILayout.Button("Log Draw Call Estimates"))
            {
                ...
                foreach (Level level in array)
                {
                    global::UnityEngine.Debug.Log("Level " + level.level + " has " + level.cells.Length + " cells and " + level.CountDrawCalls() + " draw calls");
                }
            }
        ...
        {
            int num = ((!PlatformUtils.isConsolePlatform) ? System.Math.Min(settings.maxThreads, Environment.ProcessorCount - Environment.ProcessorCount / 4) : settings.maxThreads);
            global::UnityEngine.Debug.LogFormat(this, "Starting {0} meshing threads.", num);
            for (int i = 0; i < num; i++)
            {
                ...
                {
                    int num2 = 4 + i % 2;
                    consoleAffinityMask = 1 << num2;
                    consoleAffinityMask |= 0x40;
                }
