    ...
    public class CyclopsLightingPanel : MonoBehaviour
    {
        ...
        public void ToggleFloodlights()
        {
            if (cyclopsRoot.powerRelay.GetPowerStatus() != 0 && !cyclopsRoot.silentRunning)
            {
                ...
                UpdateLightingButtons();
                if (PlatformUtils.isPS4Platform && Player.main.isPiloting && Player.main.currentSub == cyclopsRoot)
                {
                    ToggleLightbar(floodlightsOn);
                }
            }
        }
        ...
        {
            if (Player.main.currentSub != cyclopsRoot)
            {
                return;
            }
            ...
            else if (PlatformUtils.isPS4Platform)
            {
                ToggleLightbar(enable: false);
            }
        }
