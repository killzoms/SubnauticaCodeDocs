...
namespace AssemblyCSharp
{
    public class RangedAttackLastTarget : AttackLastTarget
    {
        ...
        public override void Perform(Creature creature, float deltaTime)
        {
            ...
            else if (state == State.Charge && Time.time > startCastingTime)
            {
        ...
        private void Update()
        {
            ...
            {
                Cast(directionToTarget: (!(currentAttack.projectilesSpread > 0f)) ? (currentTarget.transform.position - ammoSpawnPoint.position).normalized : (GetLookDirection() + currentAttack.projectilesSpread * global::UnityEngine.Random.onUnitSphere).normalized, attackType: currentAttack);
                projectilesCasted++;
        ...
        protected virtual void Cast(RangedAttackType attackType, Vector3 directionToTarget)
        {
            ...
            obj.GetComponent<Rigidbody>().AddForce((1f + global::UnityEngine.Random.Range(-0.05f, 0.05f)) * attackType.ammoVelocity * directionToTarget, ForceMode.VelocityChange);
