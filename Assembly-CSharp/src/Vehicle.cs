...
namespace AssemblyCSharp
{
    ...
    public abstract class Vehicle : HandTarget, IHandTarget, IProtoEventListener, IProtoTreeEventListener, IAnimParamReceiver, IQuickSlots
    {
        ...
        private int verticalVelocitySoundIndex = -1;
        ...
        protected virtual void OnDockedChanged(bool docked, DockType dockType)
        {
            ...
            LayerMask layerMask = (docked ? LayerMask.NameToLayer("Interior") : LayerMask.NameToLayer("Default"));
            Utils.SetLayerRecursively(base.gameObject, layerMask, renderersOnly: true, LayerMask.NameToLayer("BaseClipProxy"));
            if ((bool)componentInChildren)
            {
        ...
        {
            UpdateModuleSlots();
            InvokeRepeating("PassiveEnergyConsumption", global::UnityEngine.Random.value, 1f);
            InvokeRepeating("UpdateEnergyRecharge", global::UnityEngine.Random.value, 1f);
        ...
        public void OnHandHover(GUIHand hand)
        {
            if (!GetPilotingMode() && GetEnabled())
            {
                ...
                HandReticle.main.SetInteractText(handLabel);
            }
        }
        ...
        private void OnKill()
        {
            ...
            {
                GameObject obj = global::UnityEngine.Object.Instantiate(destructionEffect);
        ...
        protected virtual void OnPilotModeEnd()
        {
            ...
            useRigidbody.interpolation = RigidbodyInterpolation.None;
            pilotId = null;
        }
        ...
        private void PlaySplashSound()
        {
            ...
            if (verticalVelocitySoundIndex == -1)
            {
        ...
        private void ApplyPhysicsMove()
        {
            ...
            bool flag = base.transform.position.y < Ocean.main.GetOceanLevel() && base.transform.position.y < worldForces.waterDepth && !precursorOutOfWater;
            if (!(moveOnLand || flag))
            {
                return;
            }
            ...
            else if (controlSheme == ControlSheme.Submarine || controlSheme == ControlSheme.Mech)
            {
                ...
                float num2 = Mathf.Abs(vector3.x) * sidewardForce + Mathf.Max(0f, vector3.z) * forwardForce + Mathf.Max(0f, 0f - vector3.z) * backwardForce;
                vector3 = base.transform.rotation * vector3;
                vector3.y = 0f;
                vector3 = Vector3.Normalize(vector3);
                if (onGround)
                {
                    vector3 = Vector3.ProjectOnPlane(vector3, surfaceNormal);
                    vector3.y = Mathf.Clamp(vector3.y, -0.5f, 0.5f);
                    num2 *= onGroundForceMultiplier;
                }
                if (Application.isEditor)
                {
                    Debug.DrawLine(base.transform.position, base.transform.position + vector3 * 4f, Color.white);
                }
                Vector3 vector4 = new Vector3(0f, vector2.y, 0f);
                vector4.y *= verticalForce * Time.deltaTime;
                Vector3 acceleration2 = num2 * vector3 * Time.deltaTime + vector4;
        ...
        public virtual void TeleportVehicle(Vector3 goToPos, Quaternion dir)
        {
            useRigidbody.isKinematic = true;
        ...
        {
            ProfilingUtils.BeginSample("Vehicle.Update-Piloting");
            if (GetPilotingMode() && CanPilot() && (moveOnLand || base.transform.position.y < Ocean.main.GetOceanLevel()))
            {
                ...
                else if (controlSheme == ControlSheme.Submarine || controlSheme == ControlSheme.Mech)
                {
                    if (rotationLocked)
                    {
                        ProfilingUtils.BeginSample("SetRotationLocked");
                        float num2 = Mathf.Max(useRigidbody.angularDrag, Mathf.Abs(angularVelocity.y) * useRigidbody.angularDrag);
                        angularVelocity.y = global::UWE.Utils.Slerp(angularVelocity.y, 0f, num2 * Time.deltaTime);
                        if (vector.x != 0f)
                        {
                            angularVelocity.y += vector.x * sidewaysTorque;
                        }
                        base.transform.localEulerAngles = base.transform.localEulerAngles + angularVelocity * Time.deltaTime;
        ...
        private void StabilizeRoll()
        {
            ...
            if (num <= 178f)
            {
                ...
                useRigidbody.AddTorque(base.transform.forward * num2 * Time.deltaTime * Mathf.Sign(base.transform.eulerAngles.z - 180f), ForceMode.VelocityChange);
            }
        }
        ...
        public virtual void FixedUpdate()
        {
            ...
            if (Application.isEditor && onGround)
            {
                Debug.DrawLine(base.transform.position, base.transform.position + 3f * surfaceNormal, Color.white, 0.1f);
            }
        }
        ...
        public virtual void OnProtoDeserialize(ProtobufSerializer serializer)
        {
            ...
            {
                StorageHelper.RestoreEquipment(serializer, serializedModules, serializedModuleSlots, modules);
        ...
        {
            CraftData.GetEnergyCost(techType, out var result);
        ...
        public virtual float GetSlotCharge(int slotID)
        {
            ...
            {
                float quickSlotMaxCharge = CraftData.GetQuickSlotMaxCharge(techType);
        ...
        {
            if (slotID >= 0 && slotID < slotIDs.Length && GetQuickSlotCooldown(slotID) == 1f && GetQuickSlotType(slotID, out var techType) == QuickSlotType.Chargeable)
            {
        ...
        {
            if (activeSlot >= 0 && GetQuickSlotCooldown(activeSlot) == 1f && GetQuickSlotType(activeSlot, out var techType) == QuickSlotType.Selectable && ConsumeEnergy(techType))
            {
        ...
        {
            if (activeSlot >= 0 && GetQuickSlotCooldown(activeSlot) == 1f && GetQuickSlotType(activeSlot, out var techType) == QuickSlotType.SelectableChargeable)
            {
        ...
        {
            if (activeSlot >= 0 && GetQuickSlotCooldown(activeSlot) == 1f && GetQuickSlotType(activeSlot, out var techType) == QuickSlotType.SelectableChargeable && quickSlotCharge[activeSlot] > 0f)
            {
                OnUpgradeModuleUse(techType, activeSlot);
                quickSlotCharge[activeSlot] = 0f;
            }
        ...
        protected void ChargeModule(TechType techType, int slotID)
        {
            ...
            float num3 = quickSlotMaxCharge - num;
            bool flag = num2 >= num3;
        ...
        public static bool TorpedoShot(ItemsContainer container, TorpedoType torpedoType, Transform muzzle)
        {
            ...
            {
                GameObject obj = global::UnityEngine.Object.Instantiate(torpedoType.prefab);
