    ...
    public class CrabSnake : Creature, IPropulsionCannonAmmo
    {
        ...
        private bool TryFindCurrentMushroom()
        {
            if (currentMushroom == null)
            {
                ...
                int num = global::UWE.Utils.OverlapSphereIntoSharedBuffer(mushroomPosition, 4f);
                for (int i = 0; i < num; i++)
                {
                    CrabsnakeMushroom componentInParent = global::UWE.Utils.sharedColliderBuffer[i].GetComponentInParent<CrabsnakeMushroom>();
        ...
        {
            return GetComponent<WaterParkCreature>() != null;
        }
