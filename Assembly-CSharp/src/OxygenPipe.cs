...
namespace AssemblyCSharp
{
    ...
    public class OxygenPipe : PlayerTool, IProtoEventListener, IPipeConnection
    {
        ...
        private void PrepareGhostModel()
        {
            if (ghostModel == null)
            {
                ghostModel = global::UnityEngine.Object.Instantiate(pipePrefab).GetComponent<OxygenPipe>();
                ghostModel.isGhost = true;
                ghostModel.name = "PipeGhostModel";
                ghostModel.pickupable.isPickupable = false;
                global::UnityEngine.Object.Destroy(ghostModel.GetComponent<LargeWorldEntity>());
                global::UnityEngine.Object.Destroy(ghostModel.GetComponent<WorldForces>());
                global::UnityEngine.Object.Destroy(ghostModel.GetComponent<Rigidbody>());
                global::UnityEngine.Object.Destroy(ghostModel.GetComponent<PrefabIdentifier>());
        ...
        {
            bool result = true;
            Vector3 value = parent.GetAttachPoint() - base.transform.position;
            int num = global::UWE.Utils.RaycastIntoSharedBuffer(new Ray(GetPipePosition(parent), Vector3.Normalize(value)), value.magnitude);
            for (int i = 0; i < num; i++)
            {
                if (global::UWE.Utils.GetEntityRoot(global::UWE.Utils.sharedHitBuffer[i].collider.gameObject) != parent.GetGameObject())
                {
                    result = false;
                    break;
                }
        ...
        private void UpdateGhostAttach()
        {
            ...
            float num = 1000f;
            int num2 = global::UWE.Utils.OverlapSphereIntoSharedBuffer(base.transform.position, 11f);
            for (int i = 0; i < num2; i++)
            {
                GameObject entityRoot = global::UWE.Utils.GetEntityRoot(global::UWE.Utils.sharedColliderBuffer[i].gameObject);
                if (entityRoot == null)
                {
                    continue;
                }
        ...
        private bool PlaceInWorld(OxygenPipe pipe)
        {
            if (ghostModel.GetParent() != null)
            {
                ...
                pipe.rigidBody.isKinematic = true;
                ghostModel.gameObject.SetActive(value: false);
                return true;
            }
            return false;
