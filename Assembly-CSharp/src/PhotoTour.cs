...
namespace AssemblyCSharp
{
    public class PhotoTour : MonoBehaviour
    {
        ...
        private List<Sample> playingSamples;
        ...
        {
            string json = File.ReadAllText(SNUtils.InsideUnmanaged("phototour.json"));
        ...
        private void TakeShot()
        {
            if (bScreenShotsAllowed)
            {
                ...
                PlatformUtils.main.CaptureScreenshot(Path.Combine(sshotDir, "phototour-shot-" + @int.x + "-" + @int.y + "-" + @int.z + ".png"));
            }
        }
        ...
        {
            return Directory.GetFiles(toursFolder, "*.tour");
        }
        ...
        public void StartRecording(string tourName, int timeSeconds)
        {
            ...
            {
                recordingPath = global::UWE.Utils.GenerateNumberedFileName(Path.Combine(toursFolder, "phototour-"), ".tour");
            }
            else
            {
                recordingPath = Path.Combine(toursFolder, $"{tourName}.tour");
            }
        ...
        {
            MainCameraControl[] array = global::UnityEngine.Object.FindObjectsOfType<MainCameraControl>();
            int num = 0;
            if (num < array.Length)
            {
                MainCameraControl mainCameraControl = (cam = array[num]);
            }
        }
    }
}
