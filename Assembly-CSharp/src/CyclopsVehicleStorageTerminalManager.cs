...
namespace AssemblyCSharp
{
    public class CyclopsVehicleStorageTerminalManager : MonoBehaviour
    {
        ...
        public void VehicleDocked(Vehicle vehicle)
        {
            ...
            InvokeRepeating("UpdateText", global::UnityEngine.Random.value, 1f);
            ...
            Text componentInChildren = currentScreen.GetComponentInChildren<Text>();
        ...
        public void StorageButtonClick(VehicleStorageType type, int slotID)
        {
            ...
            switch (type)
            {
                case VehicleStorageType.Storage:
                    if (dockedVehicleType == DockedVehicleType.Seamoth)
                    {
                        SeamothStorageInput[] allComponentsInChildren2 = currentVehicle.GetAllComponentsInChildren<SeamothStorageInput>();
                        foreach (SeamothStorageInput seamothStorageInput in allComponentsInChildren2)
                        {
                ...
                {
                    ExosuitTorpedoArm[] allComponentsInChildren = currentVehicle.GetAllComponentsInChildren<ExosuitTorpedoArm>();
                    int num = 0;
                    ExosuitTorpedoArm[] array = allComponentsInChildren;
                    foreach (ExosuitTorpedoArm exosuitTorpedoArm in array)
                    {
                        if (num == slotID)
                        {
