...
namespace AssemblyCSharp
{
    ...
    public class uGUI_Toolbar : MonoBehaviour, uGUI_IIconManager, uGUI_INavigableIconGrid
    {
        ...
        public Text cycleRightBtnText;

        public Text cycleLeftBtnText;
        ...
        private Atlas.Sprite _backgroundAtlasSpriteNormal;

        private Atlas.Sprite _backgroundAtlasSpriteSelected;

        private Atlas.Sprite backgroundAtlasSpriteNormal
        {
            get
            {
                if (_backgroundAtlasSpriteNormal == null && backgroundSpriteNormal != null)
                {
                    _backgroundAtlasSpriteNormal = new Atlas.Sprite(backgroundSpriteNormal);
                }
                return _backgroundAtlasSpriteNormal;
            }
        }

        private Atlas.Sprite backgroundAtlasSpriteSelected
        {
            get
            {
                if (_backgroundAtlasSpriteSelected == null && backgroundSpriteSelected != null)
                {
                    _backgroundAtlasSpriteSelected = new Atlas.Sprite(backgroundSpriteSelected);
                }
                return _backgroundAtlasSpriteSelected;
            }
        }
        ...
        public void Select(int index)
        {
            if (index >= 0 && index < icons.Count)
            {
                ...
                if (selected >= 0)
                {
                    ...
                    uGUI_ItemIcon2.SetBackgroundSprite(backgroundAtlasSpriteNormal);
                }
                selected = index;
                uGUI_ItemIcon2 = icons[selected];
                uGUI_ItemIcon2.SetBackgroundSprite(backgroundAtlasSpriteSelected);
                if (selectionMarker != null)
                {
        ...
        public void GetTooltip(uGUI_ItemIcon icon, out string tooltipText, List<TooltipIcon> tooltipIcons)
        {
            ...
            {
                manager.GetToolbarTooltip(num, out tooltipText, tooltipIcons);
            }
        }
        ...
        private uGUI_ItemIcon CreateIcon(object content, Vector2 backgroundSize, Vector2 foregroundSize, Vector2 buttonSize, float x, float y)
        {
            ...
            if (content is Atlas.Sprite)
            {
                uGUI_ItemIcon2.SetForegroundSprite(content as Atlas.Sprite);
            }
        ...
        private void UpdateCycleBtnText()
        {
            ...
            if (cycleLeftBtnText != null)
            {
                cycleLeftBtnText.text = uGUI.FormatButton(GameInput.Button.UIPrevTab);
                cycleLeftBtnText.gameObject.SetActive(value: true);
            }
            if (cycleRightBtnText != null)
            {
                cycleRightBtnText.text = uGUI.FormatButton(GameInput.Button.UINextTab);
