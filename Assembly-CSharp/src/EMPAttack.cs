...
namespace AssemblyCSharp
{
    public class EMPAttack : CreatureAction
    {
        ...
        public override float Evaluate(Creature creature)
        {
            ...
            if ((bool)target && Time.time > timeNextBlast)
            {
        ...
        {
            ProfilingUtils.BeginSample("CreatureAction::EMPAttack");
            if (!casting && Time.time > timeNextBlast && (bool)target && target.GetIntensity() > 0f && Time.time > timeNextSwim)
            {
