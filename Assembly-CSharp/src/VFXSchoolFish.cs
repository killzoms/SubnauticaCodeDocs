...
namespace AssemblyCSharp
{
    public class VFXSchoolFish : MonoBehaviour
    {
        ...
        private void Init()
        {
            ...
            quantity = Random.Range(quantity - randomQuantity, quantity + randomQuantity);
            nextRepulsorUpdate = Time.time + Random.value * timeBetweenRepulsorUpdates;
            nextDistanceUpdate = Time.time + Random.value * timeBetweenDistanceUpdates;
        ...
        public bool IsWithinCameraFrustum()
        {
            bool result = false;
            Camera camera = MainCamera.camera;
            Vector3 vector = meshRenderer.bounds.center - camera.transform.position;
            float num = Vector3.SqrMagnitude(meshRenderer.bounds.center - meshRenderer.bounds.max);
            if (Vector3.SqrMagnitude(vector) < num)
            {
                result = true;
            }
            else if (Vector3.Dot(camera.transform.forward, vector) > 0f)
            {
                result = GeometryUtility.TestPlanesAABB(MainCamera.GetCameraFrustumPlanes(), meshRenderer.bounds);
            }
            return result;
        }
        ...
        {
            bool flag = IsWithinCameraFrustum();
            if (!Player.main.displaySurfaceWater || !flag)
            {
            ...
            else if (Player.main.displaySurfaceWater && flag)
            {
