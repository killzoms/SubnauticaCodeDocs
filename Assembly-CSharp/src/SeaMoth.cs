...
namespace AssemblyCSharp
{
    ...
    public class SeaMoth : Vehicle
    {
        ...
        public float enginePowerConsumption = 71f / (339f * (float)System.Math.PI);
        ...
        {
            ProfilingUtils.BeginSample("base.Update");
            base.Update();
            ProfilingUtils.EndSample();
            ProfilingUtils.BeginSample("UpdateSounds");
            UpdateSounds();
            ProfilingUtils.EndSample();
            ProfilingUtils.BeginSample("GetPilotingMode");
            if (GetPilotingMode())
            {
            ...
            ProfilingUtils.EndSample();
            ProfilingUtils.BeginSample("UpdateScreenFX");
            UpdateScreenFX();
            ProfilingUtils.EndSample();
            ProfilingUtils.BeginSample("UpdateDockedAnim");
            UpdateDockedAnim();
            ProfilingUtils.EndSample();
        }
        ...
        public void onLightsToggled(bool active)
        {
            ...
            {
                if (active && volumeticLights.Length != 0)
                {
                    PlatformUtils.SetLightbarColor(volumeticLights[0].color);
                }
                else
                {
        ...
        private void UpdateScreenFX()
        {
            if (GetPilotingMode())
            {
                ...
                _smoothedMoveSpeed = global::UWE.Utils.Slerp(_smoothedMoveSpeed, to, Time.deltaTime);
            }
            else
            {
