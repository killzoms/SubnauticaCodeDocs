...
namespace AssemblyCSharp
{
    ...
    public class AttachAndSuck : CreatureAction
    {
        ...
        {
            BleederAttachTarget componentInHierarchy = global::UWE.Utils.GetComponentInHierarchy<BleederAttachTarget>(collision.gameObject);
            LiveMixin component = collision.gameObject.GetComponent<LiveMixin>();
            bool flag = timeDetached + 4f < Time.time;
            if (componentInHierarchy != null && component != null && !componentInHierarchy.occupied && flag && GetComponent<LiveMixin>().IsAlive())
            {
                Player componentInHierarchy2 = global::UWE.Utils.GetComponentInHierarchy<Player>(collision.gameObject);
        ...
        {
            if (timeLastSuck + 1f < Time.time)
            {
                timeLastSuck = Time.time;
