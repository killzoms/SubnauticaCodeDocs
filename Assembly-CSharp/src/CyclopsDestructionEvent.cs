...
namespace AssemblyCSharp
{
    public class CyclopsDestructionEvent : MonoBehaviour
    {
        ...
        public void DestroyCyclops()
        {
            ...
            Invoke("SpawnLoot", swapModelsDelay + 0.5f);
            ...
            if ((bool)component)
            {
                ...
                string text2 = (component.label = ((!(subName == "")) ? $"{text}: {subName}" : text));
            }
        }
        ...
        {
            GameObject prefabForTechType = CraftData.GetPrefabForTechType(TechType.ScrapMetal);
            for (int i = 0; i < 15; i++)
            {
                ...
                global::UWE.Utils.SetIsKinematic(Object.Instantiate(prefabForTechType, lootSpawnPoint + vector, Random.rotation).GetComponent<Rigidbody>(), state: false);
            }
            prefabForTechType = CraftData.GetPrefabForTechType(TechType.ComputerChip);
            for (int j = 0; j < 5; j++)
            {
                ...
                global::UWE.Utils.SetIsKinematic(Object.Instantiate(prefabForTechType, lootSpawnPoint2 + vector2, Random.rotation).GetComponent<Rigidbody>(), state: false);
            }
        }
