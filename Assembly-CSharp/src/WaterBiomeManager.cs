...
namespace AssemblyCSharp
{
    ...
    public class WaterBiomeManager : MonoBehaviour, ICompileTimeCheckable
    {
        ...
        private Dictionary<string, int> biomeLookup = new Dictionary<string, int>();
        ...
        private void Start()
        {
            ...
            for (int i = 0; i < biomeSettings.Count; i++)
            {
                ...
                biomeSkies.Add(item);
                string key = biomeSettings[i].name.ToLower();
                ...
                {
                    biomeLookup.Add(biomeSettings[i].name.ToLower(), i);
                }
            }
        ...
        private void CreateTextures()
        {
            ...
            extinctionTextureBlurred = new RenderTexture(settingsTextureUpsampledSize, settingsTextureUpsampledSize * settingsTextureUpsampledSize, 0, RenderTextureFormat.ARGBFloat, RenderTextureReadWrite.Linear);
            extinctionTextureBlurred.wrapMode = TextureWrapMode.Clamp;
            extinctionTextureBlurred.name = "extinctionTextureBlurred";
            scatteringTextureBlurred = new RenderTexture(settingsTextureUpsampledSize, settingsTextureUpsampledSize * settingsTextureUpsampledSize, 0, RenderTextureFormat.ARGBFloat, RenderTextureReadWrite.Linear);
            scatteringTextureBlurred.wrapMode = TextureWrapMode.Clamp;
            scatteringTextureBlurred.name = "scatteringTextureBlurred";
            emissiveTextureBlurred = new RenderTexture(settingsTextureUpsampledSize, settingsTextureUpsampledSize * settingsTextureUpsampledSize, 0, RenderTextureFormat.ARGBFloat, RenderTextureReadWrite.Linear);
        ...
        private void Update()
        {
            if (LargeWorld.main == null || LargeWorld.main.Mounted())
            {
        ...
        private void UpdateForPosition(Vector3 wsCenter, bool forceRebuild)
        {
            ...
            {
                ProfilingUtils.BeginSample("Compute dirty regions");
                int numRegions = 0;
                int numRegions2 = 0;
                if (forceRebuild)
                {
                ...
                int num = 0;
                ProfilingUtils.BeginSample("Sample biomes");
                switch (numRegions2)
                {
                    case 0:
                ...
                ProfilingUtils.EndSample();
                ProfilingUtils.BeginSample("Updating GPU texture from CPU");
        ...
        private void RasterizeAtmosphereVolumes(Vector3 wsCenter, bool highDetail)
        {
            ...
            ProfilingUtils.BeginSample("RasterizeAtmosphereVolumes");
        ...
        {
            if (!string.IsNullOrEmpty(name) && biomeLookup.TryGetValue(name.ToLower(), out var value))
            {
                return value;
            }
            return -1;
        ...
        public bool GetSettings(Vector3 wsPosition, out WaterscapeVolume.Settings settings)
        {
            int num = -1;
            if ((bool)LargeWorld.main)
            {
                num = GetBiomeIndex(GetBiome(wsPosition, onlyAffectsVisuals: false));
            }
            if (num == -1)
            {
                settings = new WaterscapeVolume.Settings();
                return false;
            }
        ...
        {
            ProfilingUtils.BeginSample("GetEnclosingAtmosphereVolume");
            ProfilingUtils.BeginSample("Get overlapping triggers");
            float radius = 0f;
            int num = global::UWE.Utils.OverlapSphereIntoSharedBuffer(wsPosition, radius, atmosphereVolumesLayerMask, QueryTriggerInteraction.Collide);
            ProfilingUtils.EndSample();
            AtmosphereVolume result = null;
            if (num > 0)
            {
                ulong num2 = 0uL;
                ProfilingUtils.BeginSample("Get atmosphere volumes");
                for (int i = 0; i < num; i++)
                {
                    AtmosphereVolume component = global::UWE.Utils.sharedColliderBuffer[i].GetComponent<AtmosphereVolume>();
        ...
        private void BuildSettingsArrayTextures()
        {
            ...
            {
                global::UWE.Utils.DestroyWrap(biomeExtinctionTexture);
                global::UWE.Utils.DestroyWrap(biomeScatteringTexture);
                global::UWE.Utils.DestroyWrap(biomeEmissiveTexture);
            }
        ...
        {
            ProfilingUtils.BeginSample("GetBiomeEnvironment");
            Sky result = null;
