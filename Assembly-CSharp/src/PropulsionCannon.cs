...
namespace AssemblyCSharp
{
    public class PropulsionCannon : MonoBehaviour, IItemSelectorManager
    {
        ...
        {
            ProfilingUtils.BeginSample("PropulsionCannon.GetAABB");
            FixedBounds component = target.GetComponent<FixedBounds>();
            Bounds result = ((!(component != null)) ? global::UWE.Utils.GetEncapsulatedAABB(target, 20) : component.bounds);
        ...
        public GameObject GetNearbyGrabbedObject()
        {
        ...
        {
            ProfilingUtils.BeginSample("PropulsionCannon.CheckLineOfSight");
            bool result = true;
            int num = global::UWE.Utils.RaycastIntoSharedBuffer(a, Vector3.Normalize(b - a), (b - a).magnitude, ~(1 << LayerMask.NameToLayer("Player")), QueryTriggerInteraction.Ignore);
            bool flag = false;
            for (int i = 0; i < num; i++)
            {
                if (flag)
                ...
                ProfilingUtils.BeginSample("CheckLineOfSight.CheckIsObstacle");
                GameObject entityRoot = global::UWE.Utils.GetEntityRoot(global::UWE.Utils.sharedHitBuffer[i].collider.gameObject);
                if (!entityRoot)
                {
                    entityRoot = global::UWE.Utils.sharedHitBuffer[i].collider.gameObject;
                }
        ...
        private bool ValidateObject(GameObject go)
        {
            ...
            iammo.Clear();
            if (flag2 && energyInterface.hasCharge)
            {
                return !flag;
            }
            return false;
        ...
        {
            ProfilingUtils.BeginSample("PropulsionCannon.TraceForGrabTarget");
            Vector3 position = MainCamera.camera.transform.position;
            int layerMask = ~(1 << LayerMask.NameToLayer("Player"));
            int num = global::UWE.Utils.SpherecastIntoSharedBuffer(position, 1.2f, MainCamera.camera.transform.forward, pickupDistance, layerMask);
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                GameObject entityRoot = global::UWE.Utils.GetEntityRoot(raycastHit.collider.gameObject);
        ...
        private void FixedUpdate()
        {
            if (grabbedObject != null)
            {
                ...
                else
                {
                    ...
                    vector = (component.velocity = global::UWE.Utils.SlerpVector(vector, Vector3.zero, amount));
