...
namespace AssemblyCSharp
{
    ...
    public class SubRoot : MonoBehaviour, IProtoEventListener, IOnTakeDamage
    {
        ...
        private SubWaterPlane waterPlaneComp;
        ...
        public virtual void Awake()
        {
            ...
            subDamageSounds = global::UnityEngine.Object.Instantiate(subDamageSoundsPrefab, Vector3.zero, Quaternion.identity).GetComponent<SubDamageSounds>();
        ...
        public virtual void Start()
        {
            ...
            if ((bool)depthClearer)
            {
                depthClearer.enabled = false;
                depthClearer.castShadows = false;
                depthClearer.receiveShadows = false;
            }
        ...
        private void CrushDamageRandomPart(float amount)
        {
            ...
            {
                CrushDamageable crushDamageable = componentsInChildren[global::UnityEngine.Random.Range(0, componentsInChildren.Length)];
        ...
        private void Update()
        {
            ...
            if (waterPlane != null)
            {
                ...
                {
                    waterPlane.GetComponent<SubWaterPlane>().leakAmount = floodFraction;
                }
                else
                {
                    waterPlane.GetComponent<SubWaterPlane>().leakAmount = 0f;
                }
            }
        ...
        public void OnPlayerEntered(Player player)
        {
            if ((bool)depthClearer)
            {
                depthClearer.enabled = true;
                Material material = depthClearer.material;
                if ((bool)material)
                {
                    material.renderQueue = 2300;
                }
            }
            if ((bool)waterPlaneComp)
            {
                waterPlaneComp.OnPlayerEntered(player);
            }
            ...
            if ((bool)voiceNotificationManager && !uGUI.isLoading)
            {
        ...
        public void OnPlayerExited(Player player)
        {
            ...
            if (waterPlaneComp != null)
            {
                waterPlaneComp.OnPlayerExited(player);
            }
        ...
        {
            if ((double)floodFraction < 0.0001)
            {
                return false;
            }
            if (waterPlaneComp != null)
            {
                float y = waterPlaneComp.transform.position.y;
                return wsPos.y < y;
            }
            return false;
        }
