...
namespace AssemblyCSharp
{
    ...
    public class ShuttleCrawler : MonoBehaviour
    {
        ...
        private void Jump()
        {
            ...
            Vector3 vector = base.transform.up + new Vector3(global::UnityEngine.Random.Range(-0.1f, 0.1f), 0f, global::UnityEngine.Random.Range(-0.1f, 0.1f));
            vector.Normalize();
            GetComponent<Rigidbody>().AddForce(vector * (jumpBaseForce + global::UnityEngine.Random.value * jumpVariableForce));
        ...
        {
            SafeAnimator.SetBool(animator, "on_ground", newState);
        ...
        {
            if (debugTriggerJump && animator.GetBool("on_ground"))
            {
                Jump();
        ...
        private void Update()
        {
            ...
            SafeAnimator.SetBool(animator, "jump", value);
        ...
        private void UpdateWalking()
        {
            ...
            if (walkDuration == 0f)
            {
                if (global::UnityEngine.Random.value < walkChance)
                {
                    walkDuration = minWalkTime + global::UnityEngine.Random.value * walkVarianceTime;
                    float f = global::UnityEngine.Random.value * 2f * (float)Math.PI;
        ...
        {
            if (Time.time - timeLastCollision < 0.2f && !animator.GetBool("jump") && animator.GetBool("on_ground"))
            {
