...
namespace AssemblyCSharp
{
    ...
    public class MainCameraControl : MonoBehaviour
    {
        ...
        private void UpdateCamShake()
        {
            ...
            remainingShakeTime = global::UWE.Utils.Slerp(remainingShakeTime, 0f, Time.deltaTime);
        ...
        private void UpdateStrafeTilt()
        {
            ...
            strafeTilt = global::UWE.Utils.Slerp(strafeTilt, 0f, Time.deltaTime * 4f);
        }
        ...
        private void Update()
        {
            ...
            {
                swimCameraAnimation = Mathf.Clamp01(swimCameraAnimation + Time.deltaTime);
            }
            else
            {
                swimCameraAnimation = Mathf.Clamp01(swimCameraAnimation - Time.deltaTime);
            ...
            {
                cameraOffsetTransform.localEulerAngles = global::UWE.Utils.LerpEuler(cameraOffsetTransform.localEulerAngles, Vector3.zero, Time.deltaTime * 5f);
            }
            else
            {
                transform = cameraOffsetTransform;
                rotationY = Mathf.LerpAngle(rotationY, 0f, Time.deltaTime * 10f);
                base.transform.localEulerAngles = new Vector3(0f - rotationY, rotationX, 0f);
                cameraUPTransform.localEulerAngles = global::UWE.Utils.LerpEuler(cameraUPTransform.localEulerAngles, Vector3.zero, Time.deltaTime * 30f);
            }
            if (!XRSettings.enabled)
            {
                Vector3 localPosition = cameraOffsetTransform.localPosition;
                localPosition.z = Mathf.Clamp(localPosition.z + Time.deltaTime * num * 0.25f, 0f + camPDAZStart, camPDAZOffset + camPDAZStart);
            ...
            {
                camRotationX = Mathf.LerpAngle(camRotationX, 0f, Time.deltaTime * 2f);
                camRotationY = Mathf.LerpAngle(camRotationY, 0f, Time.deltaTime * 2f);
            ...
            else if (flag2)
            {
                if (!XRSettings.enabled)
                {
                    ...
                    if (num2)
                    {
                        camRotationX = Mathf.LerpAngle(camRotationX, 0f, Time.deltaTime * 10f);
                    }
                    if (flag6)
                    {
                        camRotationY = Mathf.LerpAngle(camRotationY, 0f, Time.deltaTime * 10f);
                    }
            ...
            if (!flag2 && GetCameraBob())
            {
                ...
                smoothedSpeed = global::UWE.Utils.Slerp(smoothedSpeed, to, Time.deltaTime);
                num3 += (Mathf.Sin(Time.time * 6f) - 1f) * (0.02f + smoothedSpeed * 0.15f) * swimCameraAnimation;
            }
            if (impactForce > 0f)
            {
                impactBob = Mathf.Min(0.9f, impactBob + impactForce * Time.deltaTime);
                impactForce -= Mathf.Max(1f, impactForce) * Time.deltaTime * 5f;
            }
            num3 -= impactBob;
            if (impactBob > 0f)
            {
