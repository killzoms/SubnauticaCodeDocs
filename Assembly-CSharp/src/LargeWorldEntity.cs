...
namespace AssemblyCSharp
{
    ...
    public class LargeWorldEntity : MonoBehaviour, IShouldSerialize, ICompileTimeCheckable
    {
        ...
        {
            if (!uGUI.isLoading && !(base.transform.GetComponent<SeaMoth>() != null) && !(base.transform.GetComponent<SubRoot>() != null) && !(base.transform.GetComponent<Constructable>() != null))
            {
                ProfilingUtils.BeginSample("Init object fader");
        ...
        private IEnumerator UpdateFading()
        {
            while (fadeTime < 0.5f)
            {
                ProfilingUtils.BeginSample("Fading");
        ...
        public void UpdateCell(LargeWorldStreamer streamer)
        {
            ...
            {
                ProfilingUtils.BeginSample("RB mgmt");
        ...
        {
            ProfilingUtils.BeginSample("LargeWorldEntity.OnAddToCell");
            if (!Application.isPlaying)
            {
                PrefabPlaceholdersGroup[] componentsInChildren = GetComponentsInChildren<PrefabPlaceholdersGroup>(includeInactive: true);
                for (int i = 0; i < componentsInChildren.Length; i++)
                {
                    componentsInChildren[i].Spawn(immediate: true, preview: true);
                }
                if (GetComponent<PrefabIdentifier>() != null)
                {
                    foreach (Transform item in base.transform)
                    {
                        global::UWE.Utils.DisableEditingRecursive(item.gameObject);
                    }
                }
            }
            ProfilingUtils.EndSample();
        ...
        public string CompileTimeCheck()
        {
            switch (cellLevel)
            {
                ...
                case CellLevel.Global:
                    ...
                    if ((bool)GetComponent<Beacon>() || (bool)GetComponent<DiveReel>() || (bool)GetComponent<DiveReelAnchor>() || (bool)GetComponent<Constructor>() || (bool)GetComponent<PipeSurfaceFloater>() || (bool)GetComponent<BasePipeConnector>() || (bool)GetComponent<Pipe>() || (bool)GetComponent<PlayerSoundTrigger>() || (bool)GetComponent<RegeneratePowerSource>() || (bool)GetComponent<SignalPing>() || (bool)GetComponent<DeployableStorage>())
                    {
                        return null;
                    }
