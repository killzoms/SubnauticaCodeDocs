...
namespace AssemblyCSharp
{
    public class ElectricalDefense : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            int num3 = global::UWE.Utils.OverlapSphereIntoSharedBuffer(base.transform.position, num);
            for (int i = 0; i < num3; i++)
            {
                Collider collider = global::UWE.Utils.sharedColliderBuffer[i];
                GameObject entityRoot = global::UWE.Utils.GetEntityRoot(collider.gameObject);
                if (entityRoot == null)
                {
