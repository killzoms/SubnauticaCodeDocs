...
namespace AssemblyCSharp
{
    public class WaterClipProxy : MonoBehaviour, ICompileTimeCheckable
    {
        ...
        [WeakAssetReferenceType(typeof(TextAsset))]
        public WeakAssetReference distanceField;
        ...
        private IEnumerator LoadAsync()
        {
            ...
            {
                TaskResult<DistanceField> result = new TaskResult<DistanceField>();
                yield return DistanceField.LoadSharedAsync(this.distanceField.Path, result);
                DistanceField distanceField = result.Get();
                ...
                Vector3 vector = distanceFieldSize * 0.5f + borderSizeScaled;
                Vector3 vector2 = (distanceFieldMin + distanceFieldMax) * 0.5f;
                if (immovable && !GetIntersectsWaterSurface(vector2, vector))
                {
                    base.gameObject.SetActive(value: false);
                    yield break;
                }
                CreateBoxMesh(vector2, vector);
            }
            else if (shape == Shape.Box)
            {
                Vector3 vector3 = Vector3.one * 0.5f + borderSizeScaled;
                Vector3 zero = Vector3.zero;
                if (immovable && !GetIntersectsWaterSurface(zero, vector3))
                {
                    base.gameObject.SetActive(value: false);
                    yield break;
                }
                CreateBoxMesh(zero, vector3);
            }
        ...
        {
            if (distanceFieldTexture != null && shape == Shape.DistanceField)
            {
                DistanceField.UnloadShared(distanceField.Path);
            }
        ...
        {
            if (shape == Shape.DistanceField && distanceField.IsValid)
            {
                Object @object = Resources.Load(distanceField.Path);
                if (@object == null)
                {
                    return $"File specified as distance field ('{distanceField.Path}') doesn't exist";
                }
                if (!(@object is TextAsset))
                {
                    return $"File specified as distance field ('{distanceField.Path}') isn't a TextAsset";
                }
            }
            return null;
        }
