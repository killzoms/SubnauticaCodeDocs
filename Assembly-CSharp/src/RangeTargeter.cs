    ...
    public class RangeTargeter : MonoBehaviour
    {
        ...
        private bool IsTargetValid(IEcoTarget ecoTarget)
        {
            ...
            if ((bool)gameObject && (gameObject.transform.position - eye.position).magnitude < range)
            {
                ...
                {
                    Debug.DrawLine(eye.position, gameObject.transform.position, Color.cyan);
                }
                global::UWE.Utils.TraceForTarget(eye.position, direction, base.gameObject, range, ref closestObj, ref position);
