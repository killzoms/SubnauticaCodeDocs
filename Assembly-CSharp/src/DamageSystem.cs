...
namespace AssemblyCSharp
{
    public class DamageSystem
    {
        ...
        public static float CalculateDamage(float damage, DamageType type, GameObject target, GameObject dealer = null)
        {
            ...
            switch (type)
            {
                ...
                case DamageType.Poison:
                    if (BehaviourData.GetBehaviourType(target) == BehaviourType.SmallFish || CraftData.GetTechType(target) == TechType.Gasopod || CraftData.GetTechType(target) == TechType.MapRoomCamera || target.GetComponent<Vehicle>() != null)
                    {
                        damage = 0f;
                    }
                    break;
        ...
        {
            int num = global::UWE.Utils.OverlapSphereIntoSharedBuffer(position, detonateRadius);
            ...
            for (int i = 0; i < num; i++)
            {
                if (global::UWE.Utils.sharedColliderBuffer[i].gameObject != sourceObject)
                {
                    LiveMixin liveMixin = Utils.FindAncestorWithComponent<LiveMixin>(global::UWE.Utils.sharedColliderBuffer[i].gameObject);
