...
namespace AssemblyCSharp
{
    public static class Utils
    {
        ...
        {
            private HashSet<GraphNode<T>> neighbors = new HashSet<GraphNode<T>>();
        ...
        {
            private HashSet<HierarchyNode<T>> children = new HashSet<HierarchyNode<T>>();
        ...
        public static GameObject genericLootPrefab
        {
            get
            {
                if (_genericLootPrefab == null)
                {
                    PrefabDatabase.TryGetPrefabForFilename("WorldEntities/Natural/LootCube", out _genericLootPrefab);
                }
                return _genericLootPrefab;
            }
        }
        ...
        {
            ProfilingUtils.BeginSample("Utils.NearlyEqual");
            try
            {
        ...
        {
            return PolarToVec2(global::UnityEngine.Random.value * 2f * (float)System.Math.PI, radius);
        }
        ...
        {
            float angle = global::UnityEngine.Random.value * 2f * (float)System.Math.PI;
            float radius = Mathf.Sqrt(global::UnityEngine.Random.value * (minRadius * minRadius - maxRadius * maxRadius) + maxRadius * maxRadius);
            return PolarToVec2(angle, radius);
        }

        public static Quaternion GetRandomYawQuat()
        {
            return Quaternion.AngleAxis(global::UnityEngine.Random.Range(-180, 180), Vector3.up);
        }
        ...
        public static List<float> GetRandomValues(int num)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                list.Add(global::UnityEngine.Random.value);
            }
            return list;
        }
        ...
        {
            return val + global::UnityEngine.Random.Range(0f - variance, variance);
        }

        public static float GetRandomValueClamped(float val, float variance, float min, float max)
        {
            return Mathf.Clamp(val + global::UnityEngine.Random.Range(0f - variance, variance), min, max);
        }

        public static Quaternion GetRandomSpinAbout(Vector3 dir)
        {
            return Quaternion.AngleAxis(Mathf.Lerp(0f, 360f, global::UnityEngine.Random.value), dir) * Quaternion.FromToRotation(Vector3.up, dir);
        }

        public static Quaternion GetRandomLimitedRotation(float maxDegs)
        {
            Vector3 onUnitSphere = global::UnityEngine.Random.onUnitSphere;
            return Quaternion.AngleAxis(maxDegs * global::UnityEngine.Random.value, onUnitSphere);
        }
        ...
        public static Vector3 SampleSphere(float radius)
        {
            ...
            do
            {
                result.x = Mathf.Lerp(0f - radius, radius, global::UnityEngine.Random.value);
                result.y = Mathf.Lerp(0f - radius, radius, global::UnityEngine.Random.value);
                result.z = Mathf.Lerp(0f - radius, radius, global::UnityEngine.Random.value);
            }
        ...
        {
            GameObject gameObject = global::UWE.Utils.InstantiateWrap(prefab, prefab.transform.position, prefab.transform.rotation);
        ...
        public static GameObject CreatePrefab(GameObject prefab, float maxDist = 12f, bool randomizeDirection = false)
        {
            ...
            {
                float num = 0.5f;
                vector = (transform.forward + transform.right * (global::UnityEngine.Random.value - 0.5f) * num / 2f + transform.up * (global::UnityEngine.Random.value - 0.5f) * num / 2f).normalized;
            }
            Vector3 vector2 = transform.position + maxDist * vector;
            Vector3 toDirection = Vector3.up;
            Vector3 vector3 = transform.position + vector * 1f;
            RaycastHit hitInfo = default(RaycastHit);
            if (Physics.Raycast(vector3, vector, out hitInfo, maxDist))
            {
                vector2 = hitInfo.point;
                toDirection = hitInfo.normal;
            }
            Debug.DrawLine(vector3, vector2, Color.green, 10f);
            GameObject gameObject = global::UnityEngine.Object.Instantiate(prefab, vector2, Quaternion.FromToRotation(Vector3.up, toDirection));
            gameObject.SetActive(value: true);
            return gameObject;
        }

        public static void CreateNPrefabs(TechType techType, float maxDist = 12f, int number = 1)
        {
            ...
            {
                CreateNPrefabs(prefabForTechType, maxDist, number);
            }
        }
        ...
        {
            Debug.DrawLine(trans.position, trans.position + scale * trans.right, Color.red);
            Debug.DrawLine(trans.position, trans.position + scale * trans.up, Color.green);
            Debug.DrawLine(trans.position, trans.position + scale * trans.forward, Color.blue);
        }
        ...
        public static void DebugDrawBox(Vector3 lsMin, Vector3 lsMax, int res, Transform transform, Color c, float time = 0f, bool depthTest = true)
        {
            ...
            Vector3 vector = transform.lossyScale.x * transform.TransformDirection(XProjection(v));
            Vector3 vector2 = transform.lossyScale.y * transform.TransformDirection(YProjection(v));
            Vector3 vector3 = transform.lossyScale.z * transform.TransformDirection(ZProjection(v));
        ...
        {
            ProfilingUtils.BeginSample("Utils.RemoveClonedName");
            string input = new Regex("[^a-zA-Z0-9]").Replace(objName, "");
            string result = new Regex("(Clone)").Replace(input, "");
            ProfilingUtils.EndSample();
            return result;
        }
        ...
        public static void AssertEditMode(string msg = "only allowed in edit mode", global::UnityEngine.Object context = null)
        {
        }
        ...
        {
            return items[global::UnityEngine.Random.Range(0, items.Length)];
        }

        public static T ChooseRandom<T>(List<T> items)
        {
            return items[global::UnityEngine.Random.Range(0, items.Count)];
        }
        ...
        {
            return global::UWE.Utils.GetSystemTime();
        }
        ...
        {
            return MainCamera.camera.transform.position + MainCamera.camera.transform.forward * global::UnityEngine.Random.Range(1f, distribution) + global::UnityEngine.Random.Range(-1f, 1f) * MainCamera.camera.transform.right * distribution / 2f;
        }

        public static GameObject PlayOneShotPS(GameObject particleSystemPrefab, Vector3 position, Quaternion orientation, Transform effectParent = null)
        {
            particleSystemPrefab.GetInstanceID();
            GameObject gameObject = global::UWE.Utils.InstantiateWrap(particleSystemPrefab, position, orientation);
            ...
            component.Play();
            global::UWE.Utils.DestroyWrap(gameObject, component.main.duration);
            return gameObject;
        }

        public static bool PlayOneShotPS(string effectPrefabName, Vector3 position, Quaternion orientation, Transform effectParent = null, float scale = 1f)
        {
            GameObject gameObject = (GameObject)Resources.Load(effectPrefabName);
            if ((bool)gameObject)
            {
                PlayOneShotPS(gameObject, position, orientation, effectParent).transform.localScale = new Vector3(scale, scale, scale);
                return true;
            }
            Debug.Log("PlayOneShotPS() - Couldn't load particle system from \"" + effectPrefabName + "\"");
            return false;
        }
        ...
        {
            ProfilingUtils.BeginSample("Utils.PlayEnvSound (string)");
        ...
        {
            ProfilingUtils.BeginSample("Utils.PlayEnvSound");
        ...
        {
            ProfilingUtils.BeginSample("Utils.PlayFMODAsset");
            if (asset != null)
            {
        ...
        public static int GetFMODParamIndex(FMODAsset asset, string paramName)
        {
            int result = -1;
            EventInstance @event = FMODUWE.GetEvent(asset);
            if (@event.hasHandle())
        ...
        {
            ProfilingUtils.BeginSample("Utils.Dist");
        ...
        public static string FormatTemperatureString(float temperature)
        {
            return Language.main.GetFormat("TemperatureFormat", temperature);
        }

        public static void AdjustSpeedScalarFromWeakness(ref float maxSpeed)
        {
            if (GameModeUtils.RequiresSurvival())
            {
                float weaknessSpeedScalar = Player.main.GetComponent<Survival>().GetWeaknessSpeedScalar();
                if (weaknessSpeedScalar < 1f)
                {
                    maxSpeed *= weaknessSpeedScalar;
                }
            }
        }
