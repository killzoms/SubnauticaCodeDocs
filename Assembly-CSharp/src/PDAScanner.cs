...
namespace AssemblyCSharp
{
    public static class PDAScanner
    {
        public struct ScanTarget
        {
            ...
            public static readonly Targeting.FilterRaycast Filter = (RaycastHit hit) => !(hit.collider == null) && Targeting.GetRoot(hit.collider.gameObject, out var _, out var _);
            ...
            public Vector3 GetRandomLocalPointInBounds()
            {
                if (transform != null)
                {
                    return new Vector3(global::UnityEngine.Random.value - 0.5f, global::UnityEngine.Random.value - 0.5f, global::UnityEngine.Random.value - 0.5f);
                }
                return Vector3.zero;
            }
        ...
        public static void Initialize(PDAData pdaData)
        {
            ...
            for (int count = scanner.Count; i < count; i++)
            {
                ...
                {
                    Debug.LogError("PDAScanner : Initialize() : TechType.None key found at index " + i);
                    continue;
                }
                if (mapping.ContainsKey(key))
                {
                    Debug.LogError(string.Concat("PDAScanner : Initialize() : Duplicate key '", key, "' found at index ", i));
                    continue;
                }
                ...
                if (!string.IsNullOrEmpty(encyclopedia) && !PDAEncyclopedia.HasEntryData(encyclopedia))
                {
                    ...
                    Debug.LogError("PDAScanner : Initialize() : '" + encyclopedia + "' is an unknown entry for PDAEncyclopedia! (index = " + i + ")");
                }
        ...
        public static void UpdateTarget(float distance, bool self)
        {
            ...
            else
            {
                ...
                Targeting.GetTarget(distance, out result, out var _, ScanTarget.Filter);
            }
        ...
        public static string GetDebugString()
        {
            ...
            stringBuilder.Append("\nKnown:\n");
            int num = 0;
            HashSet<TechType>.Enumerator enumerator4 = KnownTech.GetTech().GetEnumerator();
            while (enumerator4.MoveNext())
            {
                stringBuilder.AppendFormat("{0}, ", enumerator4.Current);
                if (num > 5)
                {
                    num = 0;
