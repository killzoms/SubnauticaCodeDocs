...
namespace AssemblyCSharp
{
    public class SeaEmperor : MonoBehaviour, IStoryGoalListener, ICompileTimeCheckable
    {
        ...
        private void Update()
        {
            if (!waitForBabies || babies.Count != babyAttachPoints.Length)
            {
                return;
            }
            bool flag = true;
        ...
        public void StartBabyInteraction()
        {
            ...
            while (enumerator.MoveNext())
            {
                ...
                current.StartCoroutine(global::UWE.Utils.LerpTransform(current.transform, Vector3.zero, Quaternion.identity, Vector3.one, interpolationDuration));
            }
