...
namespace AssemblyCSharp
{
    public class ExosuitDrillArm : MonoBehaviour, IExosuitArm
    {
        ...
        void IExosuitArm.Update(ref Quaternion aimDirection)
        {
            if (drillTarget != null)
            {
                Quaternion b = Quaternion.LookRotation(Vector3.Normalize(global::UWE.Utils.GetEncapsulatedAABB(drillTarget).center - MainCamera.camera.transform.position), Vector3.up);
        ...
        public void OnHit()
        {
            ...
            drillTarget = null;
            global::UWE.Utils.TraceFPSTargetPosition(exosuit.gameObject, 5f, ref closestObj, ref position);
            if (closestObj == null)
            {
