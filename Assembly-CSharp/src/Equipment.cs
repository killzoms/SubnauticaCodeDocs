...
namespace AssemblyCSharp
{
    ...
    public class Equipment : IItemsContainer
    {
        ...
        {
            EquipmentType equipmentType = CraftData.GetEquipmentType(newItem.item.GetTechType());
        ...
        {
            EquipmentType equipmentType = CraftData.GetEquipmentType(pickupable.GetTechType());
            EquipmentType slotType = GetSlotType(slot);
        ...
        public void RestoreEquipment(Dictionary<string, string> slots, Dictionary<string, InventoryItem> items)
        {
            ...
            foreach (KeyValuePair<string, string> slot in slots)
            {
                ...
                else
                {
                    ...
                    {
                        Debug.LogWarningFormat(value2.item, "Equipment.RestoreEquipment() warning - item {0} is no longer compatible with slot {1}, but still added to prevent data loss.", value2.item.GetTechType(), key);
                    }
