...
public class VoxelandChunk : MonoBehaviour, IVoxelandChunk, IVoxelandChunkInfo, IVoxelandChunk2, IManagedUpdateBehaviour, IManagedBehaviour
{
    ...
    public class GrassPos
    {
        ...
        public Vector3 GetVert(int i)
        {
            ...
            if (skipHiRes)
            {
                return i switch
                {
                    0 => GetMadeUpFaceVert(FaceQuadsVert0[quadNum]), 
                    1 => GetMadeUpFaceVert(FaceQuadsVert1[quadNum]), 
                    2 => GetMadeUpFaceVert(FaceQuadsVert2[quadNum]), 
                    _ => GetMadeUpFaceVert(FaceQuadsVert3[quadNum]), 
                };
            }
            return i switch
            {
                0 => verts[FaceQuadsVert0[quadNum]].pos, 
                1 => verts[FaceQuadsVert1[quadNum]].pos, 
                2 => verts[FaceQuadsVert2[quadNum]].pos, 
                _ => verts[FaceQuadsVert3[quadNum]].pos, 
            };
        }

        public void ComputeTransform(System.Random rng, VoxelandTypeBase settings)
        {
            ...
            scale = Mathf.Lerp(settings.grassMinScale, settings.grassMaxScale, (float)rng.NextDouble());
    ...
    {
        ProfilingUtils.BeginSample("BuildMesh (threadable)");
    ...
    public static void BuildMesh(IVoxelandChunk chunk, bool skipRelax, int maxTypes)
    {
        ...
        ProfilingUtils.BeginSample("Calculating mesh");
        CalculateMesh(chunk, maxTypes);
        ProfilingUtils.EndSample();
        ProfilingUtils.BeginSample("Vert List");
        GetVertsList(voxelandChunkWorkspace, flag);
        ProfilingUtils.EndSample();
        if (!voxeland.debugBlocky)
        {
            ProfilingUtils.BeginSample("SetDensityBasedPositions");
        ...
        if (!flag)
        {
            ProfilingUtils.BeginSample("PositionSecondaryVerts");
        ...
        {
            ProfilingUtils.BeginSample("Relax");
            Relax(voxelandChunkWorkspace, flag ? 0.5f : 1f);
            ProfilingUtils.EndSample();
        }
        ProfilingUtils.BeginSample("Normals");
    ...
    public void BuildLayerObjects()
    {
        ...
        ProfilingUtils.BeginSample("Layer objects");
        ...
        for (int i = 0; i < usedTypes.Count; i++)
        {
            ...
            if (!Application.isPlaying)
            {
                ...
                meshFilter2.sharedMesh.hideFlags = HideFlags.DontSave;
            }
            PushLayer(skipHiRes ? null : meshFilter.sharedMesh, meshFilter2.sharedMesh, i);
            ProfilingUtils.BeginSample("Layer post setup");
            if (!Application.isPlaying && generateCollider && i == 0)
            {
                MeshCollider meshCollider = EnsureCollision();
                meshCollider.sharedMesh = Voxeland.DuplicateMesh(meshFilter2.sharedMesh);
    ...
    public static void CalculateMesh(IVoxelandChunk chunk, int maxTypes)
    {
        ...
        {
            ProfilingUtils.BeginSample("Override CreateFaces");
            voxeland.faceCreator.CreateFaces(chunk);
            ProfilingUtils.EndSample();
        }
        else
        {
            ProfilingUtils.BeginSample("Rasterize chunk");
            voxeland.RasterizeVoxels(voxelandChunkWorkspace.rws, num - (3 << num4), num2 - (3 << num4), num3 - (3 << num4), num4);
            ProfilingUtils.EndSample();
            ProfilingUtils.BeginSample("Creating faces");
        ...
        list.Sort(comparer);
        ProfilingUtils.BeginSample("Welding");
        ...
        ProfilingUtils.EndSample();
        ProfilingUtils.BeginSample("Welding cross");
        int num17 = num5 + 4 - 1;
        for (num6 = 0; num6 < faces.Count; num6++)
        {
            VoxelandFace voxelandFace = faces[num6];
        ...
        ProfilingUtils.BeginSample("Adding neigs (linking)");
    ...
    public static void ComputeVisibleFacesInfo(IVoxelandChunk chunk)
    {
        ...
        ProfilingUtils.BeginSample("ComputeVisibleFacesInfo");
        List<VoxelandFace> faces = voxelandChunkWorkspace.faces;
        List<VoxelandFace> visibleFaces = voxelandChunkWorkspace.visibleFaces;
        visibleFaces.Clear();
        ProfilingUtils.BeginSample("Collect visible faces");
        for (int i = 0; i < faces.Count; i++)
        {
    ...
    public void PushLayer(Mesh hi, Mesh lo, int layer)
    {
        ...
        ProfilingUtils.BeginSample("PushLayer");
        List<VoxelandFace> visibleFaces = ws.visibleFaces;
        List<VoxelandVert> verts = ws.verts;
        int num = 0;
        ProfilingUtils.BeginSample("Compute layer-relevant mesh");
        ...
        ProfilingUtils.EndSample();
        ProfilingUtils.BeginSample("upload to Unity meshes");
        if (num3 == 0)
        {
            lo.Clear();
            if (hi != null)
        ...
        else if (!land.debugSkipMeshUpload)
        {
            if (hi != null)
            {
            ...
            if (!Application.isPlaying && layer == 0)
            {
                ProfilingUtils.BeginSample("create facemap");
                faceMap = new List<VoxelandCoords>();
                for (int l = 0; l < num2; l++)
                {
                    voxelandFace = ws.layerFaces[l];
                    faceMap.Add(new VoxelandCoords(this, voxelandFace));
                }
                ProfilingUtils.EndSample();
            }
        }
    ...
    public Material[] ComputeMaterialsForLayer(int layer, bool debugAllOpaque, bool useLQShader)
    {
        ...
        else if (!land.debugUseDummyMaterial)
        {
            ...
            {
                if (land.debugSolidColorMaterials && !Application.isPlaying)
                {
                    array[0] = land.typeSolidMaterials[num2];
                }
                else
                {
    ...
    public static void GrassPhase1(IVoxelandChunk chunk)
    {
        ...
        ProfilingUtils.BeginSample("Grass Phase 1");
    ...
    public static IEnumerable<GrassPos> EnumerateGrass(IVoxelandChunk chunk, VoxelandTypeBase settings, byte typeFilter, int randSeed, double reduction)
    {
        ...
        System.Random reductionRng = new System.Random(randSeed);
        System.Random placementRng = new System.Random(offsetX * 9999 + offsetY * 999 + offsetZ * 99 + randSeed * 9);
    ...
    private static void GrassPhase1(IVoxelandChunk chunk, int blockTypeId)
    {
        ...
        System.Random rng = new System.Random(num * 9999 + num2 * 999 + num3 * 99);
        ...
        {
            item.ComputeTransform(rng, voxelandBlockType);
    ...
    public void BuildGrass(GrassCB cb)
    {
        ...
        for (int i = 0; i < ws.nextGrassMesh; i++)
        {
            ...
            ProfilingUtils.BeginSample("Upload grass mesh");
    ...
    public void SetRenderersEnabled(bool enabled, bool fade)
    {
        ...
        ProfilingUtils.BeginSample("SetRenderersEnabled");
