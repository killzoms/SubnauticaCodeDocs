...
namespace UWE
{
    public class ArrayAllocator<T> : IEstimateBytes
    {
        public interface IAlloc : IEstimateBytes
        {
            int Offset { get; }
        ...
        public class Alloc : IAlloc, IEstimateBytes
        {
            ...
            public static void CopyTo(IAlloc source, IAlloc destination, int numElements)
            {
        ...
        public class MemoryPage
        {
            ...
            public MemoryPage(ArrayAllocator<T> allocator, int arraySize, byte minBucketIndex, byte maxBucketIndex, bool coalesceAllocs)
            {
                ...
                for (byte b = minBucketIndex; b <= maxBucketIndex; b = (byte)(b + 1))
                {
            ...
            public void ForceReset()
            {
                for (byte b = 0; b < buckets.Length; b = (byte)(b + 1))
                {
                    if (buckets[b] != null)
                    {
        ...
        public Alloc Allocate(int size)
        {
            if (size == 0)
            {
                return emptyAlloc;
        ...
        public void Free(Alloc allocation)
        {
            if (allocation != emptyAlloc)
            {
                allocation.Page.Free(allocation);
            }
        }
