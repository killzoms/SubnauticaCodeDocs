...
namespace UWE
{
    public class Utils
    {
        ...
        public static Bounds GetEncapsulatedAABB(GameObject go, int maxRenderers = -1)
        {
            ...
            if (checkRenderers.Count > 1)
            {
                ...
                for (int i = 1; i < num; i++)
                {
                    ...
                    {
                        ProfilingUtils.BeginSample("Bounds.Encapsulate");
        ...
        public static void SetIsKinematic(GameObject go, bool isKinematic)
        {
            Rigidbody[] componentsInChildren = go.GetComponentsInChildren<Rigidbody>();
            for (int i = 0; i < componentsInChildren.Length; i++)
            {
                componentsInChildren[i].isKinematic = isKinematic;
            }
        }

        public static void SetCollidersEnabled(GameObject go, bool enabled)
        {
            Collider[] allComponentsInChildren = go.GetAllComponentsInChildren<Collider>();
            for (int i = 0; i < allComponentsInChildren.Length; i++)
            {
                allComponentsInChildren[i].enabled = enabled;
            }
        }
        ...
        public static bool IsInsideCollider(Collider collider, Vector3 pos)
        {
            ...
            {
                result = IsInsideCollider((CapsuleCollider)collider, pos);
            }
            else if (collider is SphereCollider)
            {
                result = IsInsideCollider((SphereCollider)collider, pos);
            }
            else if (collider is BoxCollider)
            {
                result = IsInsideCollider((BoxCollider)collider, pos);
            }
            return result;
        }
        ...
        public static bool TraceHitComponentNormal<T>(GameObject ignoreObj, float maxDist, float sphereRadius, out Vector3 surfaceNormal, out Vector3 hitPoint) where T : MonoBehaviour
        {
            ...
            if ((bool)gameObject && (bool)gameObject.transform.parent && (UnityEngine.Object)gameObject.GetComponentInParent<T>() != (UnityEngine.Object)null)
            {
        ...
        public static Int3 PS4ConvertColorToLightBarRange(Color inColor)
        {
        ...
        public static Vector3 GetFacingVector(Facing f)
        {
            return f switch
            {
                Facing.North => new Vector3(0f, 0f, 1f), 
                Facing.South => new Vector3(0f, 0f, -1f), 
                Facing.East => new Vector3(1f, 0f, 0f), 
        ...
        {
            float num = Mathf.Sin(Vector2.Dot(co, new Vector2(12.9898f, 78.233f))) * 43758.5469f;
        ...
        public static Stream OpenEither(params string[] files)
        {
        ...
        public static Stream TryOpenEither(params string[] files)
        {
            foreach (string path in files)
            {
                if (FileUtils.FileExists(path))
        ...
        public static StreamReader OpenEitherText(params string[] files)
        {
            foreach (string path in files)
            {
                if (FileUtils.FileExists(path))
        ...
        public static PooledBinaryReader OpenEitherBinary(params string[] files)
        {
            foreach (string path in files)
            {
                if (FileUtils.FileExists(path))
        ...
        public static ushort[,] LoadRawU16SquareGrid(string filename)
        {
            ...
            ProfilingUtils.BeginSample("ReadBytes");
        ...
        public static void EnqueueWrap(WorkerThread thread, IWorkerTask task)
        {
            ...
            {
                thread.Enqueue(WorkerTask.ExecuteTask, task, null);
            }
            else
            {
                task.Execute();
        ...
        {
            ProfilingUtils.BeginSample("InstantiateWrap");
        ...
        {
            DumpOBJFile(path, meshBuffer.vertices.Array, meshBuffer.triangles.Array, meshBuffer.numVerts, meshBuffer.numTris, meshBuffer.vertices.Offset, meshBuffer.triangles.Offset);
        }

        public static void DumpOBJFile(string path, Vector3[] vertices, ushort[] triInds, int numVerts, int numTris, int startVert = 0, int startTri = 0)
        {
