...
public class Voxeland : MonoBehaviour, IVoxeland, ISerializationCallbackReceiver, ICompileTimeCheckable
{
    ...
    public struct RasterWorkspace
    {
        ...
        public void SetSize(int sx, int sy, int sz)
        {
            ...
            {
                ProfilingUtils.BeginSample("VoxelandRasterWorkspace");
    ...
    [NonSerialized]
    public VoxelandChunkBuilder overrideBuilder;
    ...
    public void UpdateOpaqueMaterial()
    {
        if (!opaqueMaterial)
        {
            opaqueMaterial = new Material(Shader.Find("UWE/Terrain/TerrainOpaquePass"));
        }
    }
    ...
    public bool CannotBuildDueToOverrideBuilder()
    {
        ...
        {
            return !overrideBuilder.CanBuildMore();
        }
        return false;
    }
    ...
    public void Update()
    {
        ...
        if (updateChunksEnabled)
        {
            ...
            if (dynamicRebuilding)
            {
                ...
                if (overrideBuilder != null && overrideBuilder.CanBuildMore())
                {
                    ProfilingUtils.BeginSample("Kickoff Build Threads");
                    ComputeClosestUnbuiltChunks(lsRef, lsForward, overrideBuilder.GetMaxBuildsThisFrame());
                    int num = 0;
                    while (overrideBuilder.CanBuildMore() && num < closestChunks.Count)
                    {
                        CloseChunk closeChunk = closestChunks[num];
    ...
    {
        ProfilingUtils.BeginSample("CreateChunk");
    ...
    public void BeginUndoStep()
    {
        ...
        if (currUndoStep != null)
        {
            Debug.LogError("BUG: mismatched call to BeginUndoStep!");
            return;
        }
        while (undoStack.Count > 29)
        {
            undoStack.RemoveAt(0);
        }
        currUndoStep = new UndoStep();
        undoStack.Add(currUndoStep);
        data.BeginUndoStep();
        if (antiAccumulating)
        {
            keepColliders = true;
    ...
    public void EndUndoStep()
    {
        ...
        if (currUndoStep == null)
        {
            Debug.LogError("BUG: mismatched call to EndUndoStep!");
            return;
        }
        if (antiAccumulating)
        {
            keepColliders = false;
            RebuildForUndoStep(currUndoStep);
        }
        currUndoStep.dataAction = data.EndUndoStep();
        currUndoStep = null;
    }
    ...
    public void UndoSetBlocks()
    {
        if (!Application.isPlaying)
        {
            ...
            {
                Debug.Log("(Voxeland) nothing to undo");
                return;
            }
            undoStack[undoStack.Count - 1].PerformUndo();
            RebuildForUndoStep(undoStack[undoStack.Count - 1]);
    ...
    {
        if (!Application.isPlaying && currUndoStep != null)
        {
            undoStack[undoStack.Count - 1].Add(new VoxelandCoords(sx, sy, sz));
            undoStack[undoStack.Count - 1].Add(new VoxelandCoords(ex, ey, ez));
        }
    }
    ...
    public void SetSphere(Vector3 center, Vector3 radius, bool add)
    {
        ...
        if (!Application.isPlaying && currUndoStep != null)
        {
            undoStack[undoStack.Count - 1].Add(new VoxelandCoords(num, num2, num3));
            undoStack[undoStack.Count - 1].Add(new VoxelandCoords(num4, num5, num6));
        }
    ...
    {
        ProfilingUtils.BeginSample("set cube internal");
    ...
    {
        ProfilingUtils.BeginSample("set cube internal 6");
        Bounds bounds = default(Bounds);
        bounds.SetMinMax(center - radius, center + radius);
        for (int i = 0; i < 7; i++)
        {
    ...
    {
        ProfilingUtils.BeginSample("set cube internal 0");
    ...
    public void SetCubes(VoxelandCoords[] coords, int type, bool add)
    {
        ...
        if (!Application.isPlaying && currUndoStep != null)
        {
            currUndoStep.Add(new VoxelandCoords(bounds.mins.x, bounds.mins.y, bounds.mins.z));
            currUndoStep.Add(new VoxelandCoords(bounds.maxs.x, bounds.maxs.y, bounds.maxs.z));
        }
        VoxelandData.OctNode.BlendArgs blend = new VoxelandData.OctNode.BlendArgs((!add) ? VoxelandData.OctNode.BlendOp.Subtraction : VoxelandData.OctNode.BlendOp.Union, add, 0);
    ...
    public IEnumerable<EditBlock> EditCoords(VoxelandCoords[] coords, int dataMargin)
    {
        ...
        ProfilingUtils.BeginSample("EditCoords");
        ProfilingUtils.BeginSample("Undo stuff");
        if (!Application.isPlaying && currUndoStep != null)
        {
            for (int j = 0; j < coords.Length; j++)
            {
                undoStack[undoStack.Count - 1].Add(coords[j]);
                undoStack[undoStack.Count - 1].Add(coords[j].GetNegativeNeighbor());
                undoStack[undoStack.Count - 1].Add(coords[j].GetPositiveNeighbor());
            }
        }
        ProfilingUtils.EndSample();
        ProfilingUtils.BeginSample("Raster existing densities");
        Int3.Bounds bounds = BoundsForCoords(coords);
        ...
        ProfilingUtils.EndSample();
        ProfilingUtils.BeginSample("Set new types");
        ...
        ProfilingUtils.BeginSample("OnCoordsEditsDone");
    ...
    public void PaintGrassAt(Vector3 point, Vector3 normal, VoxelandGrassType type)
    {
        if (type != null && (bool)type.grass)
        {
            ...
            if (!Application.isPlaying && currUndoStep != null)
            {
                currUndoStep.RegisterProp(gameObject);
            }
        }
    }
    ...
    public void ReplaceType(int sx, int sy, int sz, int ex, int ey, int ez, int oldType, int newType)
    {
        ...
        if (!Application.isPlaying && currUndoStep != null)
        {
            currUndoStep.Add(new VoxelandCoords(sx, sy, sz));
            currUndoStep.Add(new VoxelandCoords(ex, ey, ez));
        }
        EndUndoStep();
    ...
    public void BlurBlocks(VoxelandCoords[] coords)
    {
        ...
        for (int i = 0; i < size.x; i++)
        {
            for (int j = 0; j < size.y; j++)
            {
                for (int k = 0; k < size.z; k++)
                {
                    ...
                    float num = 0f;
                    num = (array2[i, j, k] = (array[i, j, k] = ((b != 0) ? VoxelandData.OctNode.DecodeNearDensity(b) : ((blurWS.typesGrid[i, j, k] == 0) ? (-1f) : 1f))));
                }
            }
        }
        if (smoothTweak)
        ...
        for (int l = 0; l < blurIterations; l++)
        {
            ...
            {
                int num4 = obj.x - bounds2.mins.x;
                int num5 = obj.y - bounds2.mins.y;
                int num6 = obj.z - bounds2.mins.z;
                array2[num4, num5, num6] = array[num4, num5, num6] * 0.4f + 0.1f * (array[num4 - 1, num5, num6] + array[num4 + 1, num5, num6] + array[num4, num5 - 1, num6] + array[num4, num5 + 1, num6] + array[num4, num5, num6 - 1] + array[num4, num5, num6 + 1]);
            }
        ...
        foreach (VoxelandCoords voxelandCoords in coords)
        {
            int num7 = voxelandCoords.x - bounds2.mins.x;
            int num8 = voxelandCoords.y - bounds2.mins.y;
            int num9 = voxelandCoords.z - bounds2.mins.z;
            float num10 = array[num7, num8, num9];
            byte b2 = VoxelandData.OctNode.EncodeDensity(num10);
            byte b3 = 0;
            if (num10 >= 0f)
            {
                b3 = blurWS.typesGrid[num7, num8, num9];
                if (b3 == 0)
                {
                    b3 = blurWS.typesGrid[num7, num8 - 1, num9];
                }
                if (b3 == 0)
                {
                    b3 = blurWS.typesGrid[num7, num8 + 1, num9];
                ...
                if (b3 == 0)
                {
                    b3 = blurWS.typesGrid[num7 - 1, num8, num9];
                }
                if (b3 == 0)
                {
                    b3 = blurWS.typesGrid[num7 + 1, num8, num9];
                ...
                if (b3 == 0)
                {
                    b3 = blurWS.typesGrid[num7, num8, num9 - 1];
                }
                if (b3 == 0)
                {
                    b3 = blurWS.typesGrid[num7, num8, num9 + 1];
    ...
    public void LegacyBlurBlocks(VoxelandCoords[] coords)
    {
        ...
        for (int i = 0; i < size.x; i++)
        {
            for (int j = 0; j < size.y; j++)
            {
                for (int k = 0; k < size.z; k++)
                {
                    float num3 = (array2[i, j, k] = (array[i, j, k] = ((blurWS.typesGrid[i, j, k] > 0) ? 1f : 0f)));
                }
            }
        }
        if (smoothTweak)
        ...
        foreach (VoxelandCoords voxelandCoords in coords)
        {
            ...
            byte b = 0;
            byte b2 = 0;
            bool flag = blurWS.densityGrid[num7, num8, num9] != 0;
            bool flag2 = blurWS.typesGrid[num7, num8, num9] > 0;
            bool flag3 = array[num7, num8, num9] > 0.5f;
            bool flag4 = false;
            for (int num10 = 0; num10 < 6; num10++)
            {
                int x = num7 + VoxelandChunk.VoxelandFace.dirToPosX[num10];
                int y = num8 + VoxelandChunk.VoxelandFace.dirToPosY[num10];
                int z = num9 + VoxelandChunk.VoxelandFace.dirToPosZ[num10];
            ...
            {
                b2 = blurWS.densityGrid[num7, num8, num9];
                b = blurWS.typesGrid[num7, num8, num9];
            }
            else
            {
                ...
                else
                {
                    ...
                    if (b == 0)
                    {
                        b = blurWS.typesGrid[num7, num8 - 1, num9];
                    }
                    if (b == 0)
                    {
                        b = blurWS.typesGrid[num7, num8 + 1, num9];
                    ...
                    if (b == 0)
                    {
                        b = blurWS.typesGrid[num7 - 1, num8, num9];
                    }
                    if (b == 0)
                    {
                        b = blurWS.typesGrid[num7 + 1, num8, num9];
                    ...
                    if (b == 0)
                    {
                        b = blurWS.typesGrid[num7, num8, num9 - 1];
                    }
                    if (b == 0)
                    {
                        b = blurWS.typesGrid[num7, num8, num9 + 1];
    ...
    public void RebuildRelevantChunks(int sx, int sy, int sz, int ex, int ey, int ez)
    {
        ...
        VoxelandBulkChunkBuilder voxelandBulkChunkBuilder = overrideBuilder as VoxelandBulkChunkBuilder;
    ...
    public void PrepareHighlightMaterial()
    {
        ...
        {
            highlightMaterial = new Material(Shader.Find("Voxeland/Highlight"));
    ...
    {
        ProfilingUtils.BeginSample("QueryDensityAt");
        queryWS.SetSize(1, 1, 1);
        RasterizeVoxels(queryWS, x, y, z, 0);
        ProfilingUtils.EndSample();
    ...
    public bool Edit(Ray aimRay, bool mouseDown, bool mouseDragged, bool shift, bool control, bool mouseUp, bool blockyEdit)
    {
        ...
        ProfilingUtils.BeginSample("Voxeland::Edit");
        PrepareHighlight();
        ...
        ProfilingUtils.BeginSample("highlight");
        if (flag3)
        {
        ...
        ProfilingUtils.EndSample();
        ProfilingUtils.BeginSample("edit");
    ...
    {
        if (!debugSolidColorMaterials || Application.isPlaying)
        {
            return;
        }
        System.Random random = new System.Random(42);
        typeSolidMaterials = new Material[types.Length];
        for (int i = 0; i < types.Length; i++)
        {
            if (types[i] != null && types[i].material != null)
            {
                typeSolidMaterials[i] = new Material(types[i].material);
                typeSolidMaterials[i].shader = Shader.Find("UWE/Terrain/Solid Example");
                typeSolidMaterials[i].color = new Color((float)random.NextDouble(), (float)random.NextDouble(), (float)random.NextDouble());
                Color value = new Color((float)random.NextDouble(), (float)random.NextDouble(), (float)random.NextDouble());
                typeSolidMaterials[i].SetColor(ShaderPropertyID._SpecColor, value);
                typeSolidMaterials[i].SetColor(ShaderPropertyID._Color, value);
            }
        }
    }
    ...
    {
        ProfilingUtils.BeginSample("Runtimeinit block types");
    ...
    public void Rebuild(bool buildChunksNow = false)
    {
        ...
        UpdateData();
        UpdateOpaqueMaterial();
        ProfilingUtils.BeginSample("rebuild");
        ...
        ProfilingUtils.BeginSample("chunk window alloc, win len = " + GetWindowLen());
    ...
    public void BuildChunk(int cx, int cy, int cz)
    {
        ...
        bool flag = overrideBuilder != null;
        if (CannotBuildDueToOverrideBuilder())
        {
            return;
        }
        ProfilingUtils.BeginSample("BuildChunk (" + chunkSize + ")");
        ...
        {
            chunkState.beingBuilt = true;
            overrideBuilder.Build(chunkState);
        }
        else
        {
            chunkState.beingBuilt = true;
    ...
    {
        ProfilingUtils.BeginSample("UpdateChunks");
        for (int i = 0; i < chunkWindow.Length; i++)
        {
            ChunkState chunkState = chunkWindow[i];
            if (chunkState.chunk != null)
    ...
    public bool ComputeClosestUnbuiltChunks(Vector3 lsRef, Vector3 lsForward, int count)
    {
        ...
        ProfilingUtils.BeginSample("ComputeClosestUnbuiltChunks");
