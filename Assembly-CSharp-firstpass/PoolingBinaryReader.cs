...
public class PoolingBinaryReader : IDisposable
{
    ...
    private ArrayAllocator<byte>.IAlloc m_buffer;

    private Decoder m_decoder;

    private ArrayAllocator<byte>.IAlloc m_charBytes;
    ...
    private unsafe int InternalReadChars(char[] buffer, int index, int count)
    {
        ...
        while (num2 > 0)
        {
            ...
            fixed (byte* ptr = alloc.Array)
            {
                ...
                {
                    num3 = m_decoder.GetChars((byte*)checked(unchecked((nuint)ptr) + unchecked((nuint)num4) + unchecked((nuint)alloc.Offset)), num, (char*)checked(unchecked((nuint)ptr2) + unchecked((nuint)checked(unchecked((nint)index) * (nint)2))), num2, flush: false);
                }
            }
            num2 -= num3;
            index += num3;
