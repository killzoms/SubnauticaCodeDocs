...
public class VoxelandData : ScriptableObject, IVoxelGrid
{
    ...
    public struct OctNode
    {
        ...
        public static OctNode Blend(OctNode n0, OctNode n1, BlendArgs args)
        {
            ...
            float num2 = args.operation switch
            {
                BlendOp.Union => Mathf.Max(a, num), 
                BlendOp.Intersection => Mathf.Min(a, num), 
                BlendOp.Subtraction => Mathf.Min(a, 0f - num), 
                BlendOp.Overwrite => num, 
                _ => num, 
            };
            if (num2 < 0f)
            {
    ...
    public void BeginUndoStep()
    {
        if (!Application.isPlaying)
        {
            ...
            {
                currUndoStep = new MyUndoAction(this);
            }
        }
    }
    ...
    public VoxelandUndoAction EndUndoStep()
    {
        ...
        MyUndoAction result = currUndoStep;
        currUndoStep = null;
        return result;
    }
    ...
    public void RecordUndoNode(int num)
    {
        ...
        for (int i = 0; i < currUndoStep.nodes.Count; i++)
        {
            if (currUndoStep.nums[i] == num)
            {
                return;
            }
        }
        currUndoStep.nums.Add(num);
        currUndoStep.nodes.Add(roots[num].DeepCopy());
    }
    ...
    public void SetBlock(int x, int y, int z, byte type, byte density, bool skipCollapse, bool threaded)
    {
        ...
        if (rootIndexForBlock != -1)
        {
            ...
            {
                RecordUndoNode(rootIndexForBlock);
            }
        ...
        for (int k = 0; k < iterations; k++)
        {
            ...
            for (int l = 1; l < length - 1; l++)
            {
                for (int m = 1; m < length2 - 1; m++)
                {
                    ...
                    b = (array2[l, m] = Mathf.Max(map[l, m] - map[l, m + 1], b));
